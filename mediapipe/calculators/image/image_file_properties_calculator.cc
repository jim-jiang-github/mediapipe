...
abslx::StatusOr<double> ComputeFocalLengthInPixels(int image_width,
                                                  int image_height,
...
abslx::StatusOr<ImageFileProperties> GetImageFileProperites(
    const std::string& image_bytes) {
  easyexif::EXIFInfo result;
  int code = result.parseFrom(image_bytes);
  if (code) {
    return abslx::InternalError("Error parsing EXIF, code: " +
                               std::to_string(code));
  }
