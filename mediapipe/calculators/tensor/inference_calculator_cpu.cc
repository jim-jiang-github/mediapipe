...
class InferenceCalculatorCpuImpl
 ...
 private:
  ...
  abslx::StatusOr<TfLiteDelegatePtr> MaybeCreateDelegate(CalculatorContext* cc);

  std::unique_ptr<InferenceRunner> inference_runner_;
};

abslx::Status InferenceCalculatorCpuImpl::UpdateContract(
...
abslx::Status InferenceCalculatorCpuImpl::Open(CalculatorContext* cc) {
  ASSIGN_OR_RETURN(inference_runner_, CreateInferenceRunner(cc));
  return abslx::OkStatus();
}

abslx::Status InferenceCalculatorCpuImpl::Process(CalculatorContext* cc) {
...
abslx::Status InferenceCalculatorCpuImpl::Close(CalculatorContext* cc) {
