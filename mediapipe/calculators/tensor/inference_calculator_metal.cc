...
class InferenceCalculatorMetalImpl
 ...
 private:
  abslx::Status InitInterpreter(CalculatorContext* cc);
  void AddDelegate(CalculatorContext* cc,
                   tflite::InterpreterBuilder* interpreter_builder);
  abslx::Status CreateConverters(CalculatorContext* cc);
...
abslx::Status InferenceCalculatorMetalImpl::UpdateContract(
...
abslx::Status InferenceCalculatorMetalImpl::Open(CalculatorContext* cc) {
  const auto& options = cc->Options<::mediapipe::InferenceCalculatorOptions>();
...
abslx::Status InferenceCalculatorMetalImpl::Process(CalculatorContext* cc) {
...
abslx::Status InferenceCalculatorMetalImpl::Close(CalculatorContext* cc) {
...
abslx::Status InferenceCalculatorMetalImpl::InitInterpreter(
...
abslx::Status InferenceCalculatorMetalImpl::CreateConverters(
    CalculatorContext* cc) {
  ...
  if (converter_from_BPHWC4_ == nil) {
    return abslx::InternalError("Error initializating output buffer converter");
  }
