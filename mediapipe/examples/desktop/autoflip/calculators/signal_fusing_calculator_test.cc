...
TEST(SignalFusingCalculatorTest, TwoInputNoTracking) {
  ...
  auto input_border = abslx::make_unique<bool>(false);
  runner->MutableInputs()->Index(0).packets.push_back(
      Adopt(input_border.release()).At(Timestamp(0)));

  auto input_face =
      abslx::make_unique<DetectionSet>(ParseTextProtoOrDie<DetectionSet>(
          R"pb(
            detections {
              score: 0.5
...
TEST(SignalFusingCalculatorTest, TwoInputShotLabeledTags) {
  ...
  auto input_shot = abslx::make_unique<bool>(false);
...
TEST(SignalFusingCalculatorTest, ThreeInputTracking) {
  ...
  auto input_border_0 = abslx::make_unique<bool>(false);
  runner->MutableInputs()->Index(0).packets.push_back(
  ...
  // Time one
  auto input_border_1 = abslx::make_unique<bool>(false);
  runner->MutableInputs()->Index(0).packets.push_back(
  ...
  // Time two
  auto input_border_2 = abslx::make_unique<bool>(false);
  runner->MutableInputs()->Index(0).packets.push_back(
  ...
  auto input_border_3 = abslx::make_unique<bool>(true);
  runner->MutableInputs()->Index(0).packets.push_back(
