...
abslx::Status SceneCameraMotionAnalyzer::AnalyzeSceneAndPopulateFocusPointFrames(
...
abslx::Status SceneCameraMotionAnalyzer::ToUseSteadyMotion(
...
abslx::Status SceneCameraMotionAnalyzer::ToUseSweepingMotion(
  ...
  scene_summary->set_crop_window_width(crop_window_width);
  scene_summary->set_crop_window_height(crop_window_height);
  const auto sweeping_log = abslx::StrFormat(
...
abslx::Status SceneCameraMotionAnalyzer::DecideCameraMotionType(
    const KeyFrameCropOptions& key_frame_crop_options,
...
// covers this union.
abslx::Status SceneCameraMotionAnalyzer::DecideSteadyLookAtRegion(
    const KeyFrameCropOptions& key_frame_crop_options,
    SceneKeyFrameCropSummary* scene_summary,
    SceneCameraMotion* scene_camera_motion) const {
...
abslx::Status SceneCameraMotionAnalyzer::AddFocusPointsFromCenterTypeAndWeight(
  ...
  if (type == TOPMOST_AND_BOTTOMMOST) {
    ...
    RET_CHECK_FAIL() << abslx::StrCat("Invalid FocusPointFrameType ", type);
  }
  return abslx::OkStatus();
}

abslx::Status SceneCameraMotionAnalyzer::PopulateFocusPointFrames(
  ...
  if (scene_camera_motion.has_steady_motion()) {
    ...
    return abslx::Status(StatusCode::kInvalidArgument, "Unknown motion type.");
  }
}
...
abslx::Status SceneCameraMotionAnalyzer::PopulateFocusPointFramesForTracking(
    const SceneKeyFrameCropSummary& scene_summary,
