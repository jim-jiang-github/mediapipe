...
typedef abslx::StatusOr<OutputStreamPoller> StatusOrPoller;
...
class CalculatorGraph {
 public:
  ...
  abslx::Status Initialize(CalculatorGraphConfig config,
  ...
  abslx::Status ObserveOutputStream(
  ...
  abslx::StatusOr<Packet> GetOutputSidePacket(const std::string& packet_name);
  ...
  abslx::Status StartRun(const std::map<std::string, Packet>& extra_side_packets,
  ...
  abslx::Status WaitForObservedOutput();
  ...
  // added.
  abslx::Status AddPacketToInputStream(const std::string& stream_name,
                                      const Packet& packet);
  ...
  abslx::Status SetInputStreamTimestampBound(const std::string& stream_name,
                                            Timestamp timestamp);

  // Sets the queue size of a graph input stream, overriding the graph default.
  abslx::Status SetInputStreamMaxQueueSize(const std::string& stream_name,
                                          int max_queue_size);
  ...
  abslx::Status CloseInputStream(const std::string& stream_name);

  // Closes all the graph input streams.
  abslx::Status CloseAllInputStreams();

  // Closes all the graph input streams and source calculator nodes.
  abslx::Status CloseAllPacketSources();
  ...
  void RecordError(const abslx::Status& error) ABSL_LOCKS_EXCLUDED(error_mutex_);
  ...
  bool GetCombinedErrors(abslx::Status* error_status);
  ...
  abslx::Status SetExecutor(const std::string& name,
  ...
  abslx::Status SetGpuResources(std::shared_ptr<GpuResources> resources);
#endif  // !MEDIAPIPE_DISABLE_GPU
 ...
 private:
  ...
  class GraphInputStream {
   public:
    ...
    void PrepareForRun(std::function<void(abslx::Status)> error_callback) {
  ...
  abslx::Status Initialize(std::unique_ptr<ValidatedGraphConfig> validated_graph,
                          const std::map<std::string, Packet>& side_packets);
  ...
  template <typename T>
  abslx::Status AddPacketToInputStreamInternal(const std::string& stream_name,
                                              T&& packet);
  ...
  abslx::Status SetExecutorInternal(const std::string& name,
                                   std::shared_ptr<Executor> executor);
  ...
  abslx::Status InitializeDefaultExecutor(
  ...
  abslx::Status CreateDefaultThreadPool(
      const ThreadPoolExecutorOptions* default_executor_options,
      int num_threads);
  ...
  abslx::Status InitializeExecutors();
  abslx::Status InitializePacketGeneratorGraph(
      const std::map<std::string, Packet>& side_packets);
  abslx::Status InitializeStreams();
  abslx::Status InitializeProfiler();
  abslx::Status InitializeCalculatorNodes();
  abslx::Status InitializePacketGeneratorNodes(
  ...
  abslx::Status PrepareServices();

#if !MEDIAPIPE_DISABLE_GPU
  abslx::Status MaybeSetUpGpuServiceFromLegacySidePacket(Packet legacy_sp);
  ...
  void CleanupAfterRun(abslx::Status* status) ABSL_LOCKS_EXCLUDED(error_mutex_);
  ...
  std::vector<abslx::flat_hash_set<InputStreamManager*>> full_input_streams_
      ABSL_GUARDED_BY(full_input_streams_mutex_);

  // Maps stream names to graph input stream objects.
  abslx::flat_hash_map<std::string, std::unique_ptr<GraphInputStream>>
      graph_input_streams_;

  // Maps graph input streams to their virtual node ids.
  abslx::flat_hash_map<std::string, int> graph_input_stream_node_ids_;

  // Maps graph input streams to their max queue size.
  abslx::flat_hash_map<std::string, int> graph_input_stream_max_queue_size_;
  ...
  std::vector<abslx::Status> errors_ ABSL_GUARDED_BY(error_mutex_);
