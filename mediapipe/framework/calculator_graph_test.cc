...
class TypedStatusHandler : public StatusHandler {
 public:
  ...
  static abslx::Status HandlePreRunStatus(
      const MediaPipeOptions& extendable_options,
      const PacketSet& input_side_packets,  //
      const abslx::Status& pre_run_status) {
    return abslx::OkStatus();
  }

  static abslx::Status HandleStatus(const MediaPipeOptions& extendable_options,
                                   const PacketSet& input_side_packets,  //
                                   const abslx::Status& run_status) {
    return abslx::OkStatus();
  }
};
...
class FailingPacketGenerator : public PacketGenerator {
 public:
  ...
  static abslx::Status Generate(const PacketGeneratorOptions& extendable_options,
                               const PacketSet& input_side_packets,
                               PacketSet* output_side_packets) {
    return abslx::UnknownError("this always fails.");
  }
};
    ...
    for (int i = 0; i < input_side_packets.NumEntries(); ++i) {
      if (input_side_packets.Index(i).Get<int>() > 0) {
        ...
        return abslx::UnknownError(
            abslx::StrCat("Integer ", i, " was not positive."));
      }
    }
    return abslx::OkStatus();
  }
...
class FailableStatusHandler : public StatusHandler {
 public:
  ...
  static abslx::Status HandlePreRunStatus(
      const MediaPipeOptions& extendable_options,
      const PacketSet& input_side_packets, const abslx::Status& pre_run_status) {
...
class IncrementingStatusHandler : public StatusHandler {
 public:
  ...
  static void SetPreRunStatusResult(const abslx::Status& status) {
    pre_run_status_result_ = status;
  }

  static void SetPostRunStatusResult(const abslx::Status& status) {
    post_run_status_result_ = status;
  }

 private:
  // Return values of HandlePreRunStatus() and HandleSTatus(), respectively.
  static abslx::Status pre_run_status_result_;
  static abslx::Status post_run_status_result_;
};

abslx::Status IncrementingStatusHandler::pre_run_status_result_ =
    abslx::OkStatus();
abslx::Status IncrementingStatusHandler::post_run_status_result_ =
    abslx::OkStatus();
...
void RunComprehensiveTest(CalculatorGraph* graph,
  ...
  for (int iteration = 0; iteration < 2; ++iteration) {
    ...
    // the 5's).
    EXPECT_EQ(abslx::StrCat(Timestamp::PostStream().DebugString(), " 1815 2006"),
    ...
    EXPECT_EQ(abslx::StrCat(Timestamp::PostStream().DebugString(), " 171 112"),
...
TEST(CalculatorGraph, LayerOrdering) {
  ...
  auto graph = abslx::make_unique<CalculatorGraph>();
...
TEST(CalculatorGraph, StatusHandlerInputVerification) {
  ...
  graph.reset(new CalculatorGraph());
  abslx::Status status = graph->Initialize(config);
...
TEST(CalculatorGraph, HandlersRun) {
  ...
  IncrementingStatusHandler::SetPreRunStatusResult(
      abslx::InternalError("Fail at pre-run"));
  graph.reset(new CalculatorGraph());
  MP_ASSERT_OK(graph->Initialize(config));
  ResetCounters(&input_side_packets);
  run_status = graph->Run(input_side_packets);
  EXPECT_TRUE(run_status.code() == abslx::StatusCode::kInternal);
  ...
  IncrementingStatusHandler::SetPreRunStatusResult(abslx::OkStatus());
  IncrementingStatusHandler::SetPostRunStatusResult(
      abslx::InternalError("Fail at post-run"));
  graph.reset(new CalculatorGraph());
  MP_ASSERT_OK(graph->Initialize(config));
  ResetCounters(&input_side_packets);
  run_status = graph->Run(input_side_packets);
...
// abslx::StatusCode::kFailedPrecondition error.
TEST(CalculatorGraph, SetOffsetInProcess) {
  ...
  EXPECT_EQ(abslx::StatusCode::kFailedPrecondition, status.code());
}
...
TEST(CalculatorGraph, TerminatesOnCancelWithOpenGraphInputStreams) {
  ...
  EXPECT_EQ(status.code(), abslx::StatusCode::kCancelled);
}
...
TEST(CalculatorGraph, AddPacketNoBusyLoop) {
  ...
  for (int i = 0; i < out_packets.size(); ++i) {
    std::string format = (i % kDecimationRatio == 0) ? "Timestamp($0) $0 $0"
                                                     : "Timestamp($0) empty $0";
    std::string expected = abslx::Substitute(format, i);
...
abslx::Status DoProcess(const InputStreamShardSet& inputs,
  ...
  MP_ASSERT_OK(graph.ObserveOutputStream(
      "count", [&count_packets](const Packet& packet) {
        ...
        if (count_packets.size() >= fail_count) {
          return abslx::UnknownError("Expected. MagicString-eatnhuea");
        } else {
          return abslx::OkStatus();
        }
      }));
...
TEST(CalculatorGraph, ObserveOutputStreamNonexistent) {
  ...
  std::vector<Packet> count_packets;  // Packets from the output stream "count".
  abslx::Status status = graph.ObserveOutputStream(
...
TEST(CalculatorGraph, ReuseValidatedGraphConfig) {
  ...
  abslx::FixedArray<CalculatorGraph> graphs(30);
...
TEST(CalculatorGraph, GraphInputStreamBeforeStartRun) {
  ...
  ASSERT_EQ(graph
                ...
                .code(),
            abslx::StatusCode::kFailedPrecondition);
}
...
class TimestampBoundTestCalculator : public CalculatorBase {
 public:
  ...
  abslx::Status Process(CalculatorContext* cc) final {
    ...
    abslx::SleepFor(abslx::Milliseconds(3));
    ++count_;
    if (count_ == 110) {
      return tool::StatusStop();
    }
...
TEST(CalculatorGraph, SimpleMuxCalculatorWithCustomInputStreamHandler) {
  ...
  abslx::Status run_status = graph.WaitUntilIdle();
  EXPECT_THAT(
      run_status.ToString(),
      testing::AllOf(
          // The core problem.
...
void DoTestMultipleGraphRuns(abslx::string_view input_stream_handler,
                             bool select_packet) {
  std::string graph_proto = abslx::StrFormat(R"(
    input_stream: 'input'
    input_stream: 'select'
    node {
      calculator: 'PassThroughCalculator'
