...
class GraphOutputStream {
 public:
  ...
  abslx::Status Initialize(const std::string& stream_name,
...
class OutputStreamObserver : public GraphOutputStream {
 ...
 private:
  ...
  std::function<abslx::Status(const Packet&)> packet_callback_;
};
...
class OutputStreamPollerImpl : public GraphOutputStream {
 ...
 private:
  abslx::Mutex mutex_;
  abslx::CondVar handler_condvar_ ABSL_GUARDED_BY(mutex_);
