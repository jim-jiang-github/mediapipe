...
class RangeCalculator : public CalculatorBase {
 public:
  ...
  abslx::Status Open(CalculatorContext* cc) final {
    ...
    // this one failed.
    if (k_ == 0) {
      return abslx::Status(abslx::StatusCode::kCancelled, "k_ == 0");
    }
    cc->Outputs().Index(0).SetHeader(
        Adopt(new std::string(abslx::StrCat(cc->CalculatorType(), k_))));
...
class MergeCalculator : public CalculatorBase {
 public:
  ...
  abslx::Status Open(CalculatorContext* cc) final {
    auto header = abslx::make_unique<std::string>();
    for (auto& input : cc->Inputs()) {
      if (!input.Header().IsEmpty()) {
        if (!header->empty()) {
          abslx::StrAppend(header.get(), " ");
        }
        abslx::StrAppend(header.get(), input.Header().Get<std::string>());
      }
    }
  ...
  abslx::Status Process(CalculatorContext* cc) final {
    ...
    if (cc->InputTimestamp().IsSpecialValue()) {
      abslx::StrAppend(&result, cc->InputTimestamp().DebugString());
    } else {
      abslx::StrAppend(&result, "Timestamp(", cc->InputTimestamp().DebugString(),
                      ")");
    }
    for (const auto& input : cc->Inputs()) {
      ...
      if (!packet.IsEmpty()) {
        abslx::StrAppend(&result, packet.Get<int>());
      } else {
...
class RandomMatrixCalculator : public CalculatorBase {
 public:
  ...
  abslx::Status Open(CalculatorContext* cc) override {
    ...
    random_ = abslx::make_unique<RandomEngine>(seed[0]);
    return abslx::OkStatus();
  }
...
typedef std::function<abslx::Status(CalculatorContext* cc)>
...
class PassThroughWithSleepCalculator : public CalculatorBase {
 public:
  ...
  abslx::Status Open(CalculatorContext* cc) final {
    ...
    if (sleep_micros_ < 0) {
      return abslx::InternalError("SLEEP_MICROS should be >= 0");
    }
    clock_ =
  ...
  abslx::Status Process(CalculatorContext* cc) final {
    clock_->Sleep(abslx::Microseconds(sleep_micros_));
    int value = cc->Inputs().Index(0).Value().Get<int>();
