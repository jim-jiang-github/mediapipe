...
constexpr abslx::string_view kGoogleProtobufAny = "google.protobuf.Any";
...
abslx::Status WriteField(const FieldData& packet, const FieldDescriptor* field,
                        std::string* result) {
  return ProtoUtilLite::WriteValue(packet, field->type(), result);
}

// Deserializes a packet from a protobuf field.
abslx::Status ReadField(abslx::string_view bytes, const FieldDescriptor& field,
                       FieldData* result) {
...
abslx::Status GetFieldValue(const FieldData& message_data,
...
abslx::Status SetFieldValue(FieldData& result, const FieldPathEntry& entry,
  ...
  if (index > field_count) {
    return abslx::OutOfRangeError(
        abslx::StrCat("Option field index out of range: ", index));
  }
...
abslx::Status SetFieldValues(FieldData& result, const FieldPathEntry& entry,
                            const std::vector<FieldData>& values) {
  if (entry.field == nullptr) {
    return abslx::InvalidArgumentError("Field not found.");
  }
  FieldType field_type = entry.field->type();
...
const FieldDescriptor* FindExtensionField(const FieldData& message_data,
                                          abslx::string_view extension_type) {
...
abslx::StatusOr<FieldData> MergeMessages(const FieldData& base,
  ...
  if (over.message_value().type_url() != base.message_value().type_url()) {
    return abslx::InvalidArgumentError(
        abslx::StrCat("Cannot merge field data with message types: ",
                     base.message_value().type_url(), ", ",
                     over.message_value().type_url()));
  }
  abslx::Cord merged_value;
...
abslx::Status FindExtension(const FieldData& message_data,
                           FieldPathEntry* entry) {
  ...
  return abslx::NotFoundError(
      abslx::StrCat("Option extension not found: ", extension_type));
}
...
abslx::StatusOr<std::vector<FieldData>> GetFieldValues(
  ...
  if ((int)results.size() <= index) {
    return abslx::OutOfRangeError(abslx::StrCat(
...
abslx::Status SetFieldValues(FieldData& message_data,
  ...
  if (field_path.empty()) {
    if (values.empty()) {
      return abslx::InvalidArgumentError("Missing field value.");
    }
...
abslx::Status SetField(FieldData& message_data, const FieldPath& field_path,
                      const FieldData& value) {
