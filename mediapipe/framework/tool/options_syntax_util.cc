 ...
 public:
  ...
  abslx::string_view Find(abslx::string_view text, size_t pos) const {
    while (pos < text.length()) {
      ...
      p = (p == abslx::string_view::npos) ? text.length() : p;
...
class OptionsSyntaxUtil::OptionsSyntaxHelper {
 public:
  ...
  abslx::string_view OptionFieldPacket(abslx::string_view name) { return name; }

  // Returns the option protobuf field name for a tag or packet name.
  abslx::string_view OptionFieldName(abslx::string_view name) { return name; }

  // Return the extension-type specified for an option field.
  abslx::string_view ExtensionType(abslx::string_view option_name) {
    constexpr abslx::string_view kExt = "Ext::";
    if (abslx::StartsWithIgnoreCase(option_name, kExt)) {
  ...
  std::vector<abslx::string_view> OptionTagNames(abslx::string_view tag) {
    if (abslx::StartsWith(tag, syntax_.tag_name)) {
      tag = tag.substr(syntax_.tag_name.length());
    } else if (abslx::StartsWith(tag, syntax_.packet_name)) {
      tag = tag.substr(syntax_.packet_name.length());
    }
    if (abslx::StartsWith(tag, syntax_.separator)) {
      tag = tag.substr(syntax_.separator.length());
    }
    return abslx::StrSplit(tag, syntax_.separator);
  }

  // Returns the field-path for an option stream-tag.
  FieldPath OptionFieldPath(abslx::string_view tag,
                            const Descriptor* descriptor) {
    std::vector<abslx::string_view> name_tags = OptionTagNames(tag);
    FieldPath result;
    for (abslx::string_view name_tag : name_tags) {
      if (name_tag.empty()) {
        continue;
      }
      abslx::string_view option_name = OptionFieldName(name_tag);
      int index;
      if (abslx::SimpleAtoi(option_name, &index)) {
  ...
  std::string GraphOptionName(const std::string& option_name) {
    std::string packet_prefix =
        syntax_.packet_name + abslx::AsciiStrToLower(syntax_.separator);
    return abslx::StrCat(packet_prefix, option_name);
  }

  // Returns the tag name for a graph option.
  std::string OptionTagName(const std::string& option_name) {
    return abslx::StrCat(syntax_.tag_name, syntax_.separator,
                        OptionFieldTag(option_name));
  }

  // Converts slash-separated field names into a tag name.
  std::string OptionFieldsTag(abslx::string_view option_names) {
    std::string tag_prefix = syntax_.tag_name + syntax_.separator;
    std::vector<abslx::string_view> names = abslx::StrSplit(option_names, '/');
    ...
    std::string result;
    std::string sep = "";
    for (abslx::string_view v : names) {
      abslx::StrAppend(&result, sep, OptionFieldTag(std::string(v)));
...
std::string OptionsSyntaxUtil::OptionFieldsTag(abslx::string_view option_names) {
  return syntax_helper_->OptionFieldsTag(option_names);
}

OptionsSyntaxUtil::FieldPath OptionsSyntaxUtil::OptionFieldPath(
    abslx::string_view tag, const Descriptor* descriptor) {
  return syntax_helper_->OptionFieldPath(tag, descriptor);
}
std::vector<abslx::string_view> OptionsSyntaxUtil::StrSplitTags(
    abslx::string_view tag_and_name) {
  return abslx::StrSplit(tag_and_name, SingleColonDelimiter());
}
