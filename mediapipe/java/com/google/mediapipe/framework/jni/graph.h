...
class Graph {
 public:
  ...
  abslx::Status AddCallbackHandler(std::string output_stream_name,
                                  jobject java_callback);
  // Adds a callback for multiple output streams.
  abslx::Status AddMultiStreamCallbackHandler(
  ...
  abslx::Status LoadBinaryGraph(std::string path_to_graph);
  // Loads a binary graph from a buffer.
  abslx::Status LoadBinaryGraph(const char* data, int size);
  // Loads a binary graph template from a buffer.
  abslx::Status LoadBinaryGraphTemplate(const char* data, int size);
  // Specifies the CalculatorGraphConfig::type of the top level graph.
  abslx::Status SetGraphType(std::string graph_type);
  // Specifies options such as template arguments for the graph.
  abslx::Status SetGraphOptions(const char* data, int size);
  ...
  abslx::Status RunGraphUntilClose(JNIEnv* env);
  ...
  abslx::Status StartRunningGraph(JNIEnv* env);
  // Closes one input stream.
  abslx::Status CloseInputStream(std::string stream_name);
  ...
  abslx::Status WaitUntilDone(JNIEnv* env);
  // Waits util graph is idle.
  abslx::Status WaitUntilIdle(JNIEnv* env);
  ...
  abslx::Status SetTimestampAndMovePacketToInputStream(
  ...
  abslx::Status SetParentGlContext(int64 java_gl_context);
 ...
 private:
  ...
  abslx::Status InitializeGraph(CalculatorGraph* graph);
  ...
  std::unordered_map<internal::PacketWithContext*,
      ...
      all_packets_;
  abslx::Mutex all_packets_mutex_;
