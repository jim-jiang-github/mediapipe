...
- (instancetype)initWithStatus:(const abslx::Status &)status {
  self = [super init];
  if (self) {
    _status = status;
  }
...
- (abslx::Status)gus_status {
  ...
  return abslx::Status(abslx::StatusCode::kUnknown, self.localizedDescription.UTF8String);
}
