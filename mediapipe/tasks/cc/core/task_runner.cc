...
abslx::StatusOr<Timestamp> ValidateAndGetPacketTimestamp(
    const PacketMap& packet_map) {
  if (packet_map.empty()) {
    return CreateStatusWithPayload(
        abslx::StatusCode::kInvalidArgument, "The provided packet map is empty.",
  ...
  for (const auto& [name, packet] : packet_map) {
    if (packet.Timestamp() != timestamp) {
      return CreateStatusWithPayload(
          ...
          abslx::Substitute("The packets in the packet map have inconsistent "
                           "timestamps: $0 and $1.",
...
/* static */
abslx::StatusOr<std::unique_ptr<TaskRunner>> TaskRunner::Create(
    CalculatorGraphConfig config,
    std::unique_ptr<tflite::OpResolver> op_resolver,
    PacketsCallback packets_callback) {
  auto task_runner = abslx::WrapUnique(new TaskRunner(packets_callback));
  MP_RETURN_IF_ERROR(
...
abslx::Status TaskRunner::Initialize(
    CalculatorGraphConfig config,
    std::unique_ptr<tflite::OpResolver> op_resolver) {
  if (initialized_) {
    return CreateStatusWithPayload(
...
abslx::Status TaskRunner::Start() {
  if (!initialized_) {
    return CreateStatusWithPayload(
        abslx::StatusCode::kInvalidArgument, "Task runner is not initialized.",
        MediaPipeTasksStatus::kRunnerFailsToStartError);
  }
  if (is_running_) {
    return CreateStatusWithPayload(
        abslx::StatusCode::kInvalidArgument, "Task runner is already running.",
        MediaPipeTasksStatus::kRunnerFailsToStartError);
  }
  {
...
abslx::StatusOr<PacketMap> TaskRunner::Process(PacketMap inputs) {
  if (!is_running_) {
    return CreateStatusWithPayload(
        abslx::StatusCode::kInvalidArgument,
  ...
  for (auto& [stream_name, packet] : inputs) {
    MP_RETURN_IF_ERROR(AddPayload(
        ...
        abslx::StrCat("Failed to add packet to the graph input stream: ",
                     stream_name),
        MediaPipeTasksStatus::kRunnerUnexpectedInputError));
  }
  last_seen_ = input_timestamp;
  if (!graph_.WaitUntilIdle().ok()) {
    abslx::Status graph_status;
...
abslx::Status TaskRunner::Send(PacketMap inputs) {
  if (!is_running_) {
    return CreateStatusWithPayload(
        abslx::StatusCode::kInvalidArgument,
        "Task runner is currently not running.",
  ...
  for (auto& [stream_name, packet] : inputs) {
    MP_RETURN_IF_ERROR(AddPayload(
        graph_.AddPacketToInputStream(stream_name,
                                      std::move(packet).At(input_timestamp)),
        abslx::Substitute("Failed to add packet to the graph input stream: $0 "
                         "at timestamp: $1",
...
abslx::Status TaskRunner::Restart() {
