...
abslx::Status RecursivelyCreateDir(abslx::string_view path,
  ...
  std::vector<std::string> path_comp = abslx::StrSplit(path, '/');
  if (path[0] == '/') {
...
abslx::Status Exists(abslx::string_view path, const file::Options& ignored) {
  struct stat64 stat_buf;
  int statval = lstat64(std::string(path).c_str(), &stat_buf);
  if (statval == 0) {
    return abslx::OkStatus();
  } else {
    return abslx::Status(abslx::StatusCode::kNotFound, "Could not stat file.");
  }
}

abslx::Status IsDirectory(abslx::string_view path,
  ...
  if (is_dir) {
    ...
    return abslx::Status(abslx::StatusCode::kNotFound, "File does not exists");
  } else {
    return abslx::Status(abslx::StatusCode::kNotFound, "Not a directory");
  }
}
