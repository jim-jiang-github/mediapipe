...
TEST_F(NonMutatingTest, Distance) {
  EXPECT_EQ(container_.size(), abslx::c_distance(container_));
  EXPECT_EQ(sequence_.size(), abslx::c_distance(sequence_));
  EXPECT_EQ(vector_.size(), abslx::c_distance(vector_));
  EXPECT_EQ(ABSL_ARRAYSIZE(array_), abslx::c_distance(array_));

  // Works with a temporary argument.
  EXPECT_EQ(vector_.size(), abslx::c_distance(std::vector<int>(vector_)));
}
...
TEST_F(NonMutatingTest, Distance_OverloadedBeginEnd) {
  ...
  EXPECT_EQ(3, abslx::c_distance(a));
  EXPECT_EQ(3, abslx::c_distance(b));
...
TEST_F(NonMutatingTest, ForEach) {
  AccumulateCalls c = abslx::c_for_each(container_, AccumulateCalls());
  ...
  AccumulateCalls c2 =
      abslx::c_for_each(std::unordered_set<int>(container_), AccumulateCalls());
  std::sort(c2.calls.begin(), c2.calls.end());
  EXPECT_EQ(vector_, c2.calls);
}

TEST_F(NonMutatingTest, FindReturnsCorrectType) {
  auto it = abslx::c_find(container_, 3);
  EXPECT_EQ(3, *it);
  abslx::c_find(abslx::implicit_cast<const std::list<int>&>(sequence_), 3);
}

TEST_F(NonMutatingTest, FindIf) { abslx::c_find_if(container_, Predicate); }

TEST_F(NonMutatingTest, FindIfNot) {
  abslx::c_find_if_not(container_, Predicate);
}

TEST_F(NonMutatingTest, FindEnd) {
  abslx::c_find_end(sequence_, vector_);
  abslx::c_find_end(vector_, sequence_);
}

TEST_F(NonMutatingTest, FindEndWithPredicate) {
  abslx::c_find_end(sequence_, vector_, BinPredicate);
  abslx::c_find_end(vector_, sequence_, BinPredicate);
}

TEST_F(NonMutatingTest, FindFirstOf) {
  abslx::c_find_first_of(container_, sequence_);
  abslx::c_find_first_of(sequence_, container_);
}

TEST_F(NonMutatingTest, FindFirstOfWithPredicate) {
  abslx::c_find_first_of(container_, sequence_, BinPredicate);
  abslx::c_find_first_of(sequence_, container_, BinPredicate);
}

TEST_F(NonMutatingTest, AdjacentFind) { abslx::c_adjacent_find(sequence_); }

TEST_F(NonMutatingTest, AdjacentFindWithPredicate) {
  abslx::c_adjacent_find(sequence_, BinPredicate);
}

TEST_F(NonMutatingTest, Count) { EXPECT_EQ(1, abslx::c_count(container_, 3)); }

TEST_F(NonMutatingTest, CountIf) {
  EXPECT_EQ(2, abslx::c_count_if(container_, Predicate));
  const std::unordered_set<int>& const_container = container_;
  EXPECT_EQ(2, abslx::c_count_if(const_container, Predicate));
}

TEST_F(NonMutatingTest, Mismatch) {
  // Testing necessary as abslx::c_mismatch executes logic.
  {
    auto result = abslx::c_mismatch(vector_, sequence_);
    EXPECT_EQ(result.first, vector_.end());
    EXPECT_EQ(result.second, sequence_.end());
  }
  {
    auto result = abslx::c_mismatch(sequence_, vector_);
...
TEST_F(NonMutatingTest, MismatchWithPredicate) {
  // Testing necessary as abslx::c_mismatch executes logic.
  {
    auto result = abslx::c_mismatch(vector_, sequence_, BinPredicate);
    EXPECT_EQ(result.first, vector_.begin());
    EXPECT_EQ(result.second, sequence_.begin());
  }
  {
    auto result = abslx::c_mismatch(sequence_, vector_, BinPredicate);
...
TEST_F(NonMutatingTest, Equal) {
  EXPECT_TRUE(abslx::c_equal(vector_, sequence_));
  EXPECT_TRUE(abslx::c_equal(sequence_, vector_));
  EXPECT_TRUE(abslx::c_equal(sequence_, array_));
  EXPECT_TRUE(abslx::c_equal(array_, vector_));

  // Test that behavior appropriately differs from that of equal().
  std::vector<int> vector_plus = {1, 2, 3};
  vector_plus.push_back(4);
  EXPECT_FALSE(abslx::c_equal(vector_plus, sequence_));
  EXPECT_FALSE(abslx::c_equal(sequence_, vector_plus));
  EXPECT_FALSE(abslx::c_equal(array_, vector_plus));
}

TEST_F(NonMutatingTest, EqualWithPredicate) {
  EXPECT_TRUE(abslx::c_equal(vector_, sequence_, Equals));
  EXPECT_TRUE(abslx::c_equal(sequence_, vector_, Equals));
  EXPECT_TRUE(abslx::c_equal(array_, sequence_, Equals));
  EXPECT_TRUE(abslx::c_equal(vector_, array_, Equals));

  // Test that behavior appropriately differs from that of equal().
  std::vector<int> vector_plus = {1, 2, 3};
  vector_plus.push_back(4);
  EXPECT_FALSE(abslx::c_equal(vector_plus, sequence_, Equals));
  EXPECT_FALSE(abslx::c_equal(sequence_, vector_plus, Equals));
  EXPECT_FALSE(abslx::c_equal(vector_plus, array_, Equals));
}
...
TEST_F(NonMutatingTest, IsPermutation) {
  ...
  EXPECT_TRUE(abslx::c_is_permutation(vector_permut_, sequence_));
  EXPECT_TRUE(abslx::c_is_permutation(sequence_, vector_permut_));

  // Test that behavior appropriately differs from that of is_permutation().
  std::vector<int> vector_plus = {1, 2, 3};
  vector_plus.push_back(4);
  EXPECT_FALSE(abslx::c_is_permutation(vector_plus, sequence_));
  EXPECT_FALSE(abslx::c_is_permutation(sequence_, vector_plus));
}

TEST_F(NonMutatingTest, IsPermutationWithPredicate) {
  auto vector_permut_ = vector_;
  std::next_permutation(vector_permut_.begin(), vector_permut_.end());
  EXPECT_TRUE(abslx::c_is_permutation(vector_permut_, sequence_, Equals));
  EXPECT_TRUE(abslx::c_is_permutation(sequence_, vector_permut_, Equals));

  // Test that behavior appropriately differs from that of is_permutation().
  std::vector<int> vector_plus = {1, 2, 3};
  vector_plus.push_back(4);
  EXPECT_FALSE(abslx::c_is_permutation(vector_plus, sequence_, Equals));
  EXPECT_FALSE(abslx::c_is_permutation(sequence_, vector_plus, Equals));
}

TEST_F(NonMutatingTest, Search) {
  abslx::c_search(sequence_, vector_);
  abslx::c_search(vector_, sequence_);
  abslx::c_search(array_, sequence_);
}

TEST_F(NonMutatingTest, SearchWithPredicate) {
  abslx::c_search(sequence_, vector_, BinPredicate);
  abslx::c_search(vector_, sequence_, BinPredicate);
}

TEST_F(NonMutatingTest, SearchN) { abslx::c_search_n(sequence_, 3, 1); }

TEST_F(NonMutatingTest, SearchNWithPredicate) {
  abslx::c_search_n(sequence_, 3, 1, BinPredicate);
}

TEST_F(NonMutatingTest, LowerBound) {
  std::list<int>::iterator i = abslx::c_lower_bound(sequence_, 3);
...
TEST_F(NonMutatingTest, LowerBoundWithPredicate) {
  ...
  std::vector<int>::iterator i = abslx::c_lower_bound(v, 3, std::greater<int>());
  EXPECT_TRUE(i == v.begin());
  EXPECT_EQ(3, *i);
}

TEST_F(NonMutatingTest, UpperBound) {
  std::list<int>::iterator i = abslx::c_upper_bound(sequence_, 1);
  ASSERT_TRUE(i != sequence_.end());
...
TEST_F(NonMutatingTest, UpperBoundWithPredicate) {
  std::vector<int> v(vector_);
  std::sort(v.begin(), v.end(), std::greater<int>());
  std::vector<int>::iterator i = abslx::c_upper_bound(v, 1, std::greater<int>());
...
TEST_F(NonMutatingTest, EqualRange) {
  std::pair<std::list<int>::iterator, std::list<int>::iterator> p =
      abslx::c_equal_range(sequence_, 2);
...
TEST_F(NonMutatingTest, EqualRangeArray) {
  auto p = abslx::c_equal_range(array_, 2);
...
TEST_F(NonMutatingTest, EqualRangeWithPredicate) {
  ...
  std::pair<std::vector<int>::iterator, std::vector<int>::iterator> p =
      abslx::c_equal_range(v, 2, std::greater<int>());
  EXPECT_EQ(1, std::distance(v.begin(), p.first));
  EXPECT_EQ(2, std::distance(v.begin(), p.second));
}

TEST_F(NonMutatingTest, BinarySearch) {
  EXPECT_TRUE(abslx::c_binary_search(vector_, 2));
  EXPECT_TRUE(abslx::c_binary_search(std::vector<int>(vector_), 2));
}

TEST_F(NonMutatingTest, BinarySearchWithPredicate) {
  std::vector<int> v(vector_);
  std::sort(v.begin(), v.end(), std::greater<int>());
  EXPECT_TRUE(abslx::c_binary_search(v, 2, std::greater<int>()));
  EXPECT_TRUE(
      abslx::c_binary_search(std::vector<int>(v), 2, std::greater<int>()));
}

TEST_F(NonMutatingTest, MinElement) {
  std::list<int>::iterator i = abslx::c_min_element(sequence_);
  ASSERT_TRUE(i != sequence_.end());
  EXPECT_EQ(*i, 1);
}
...
TEST_F(NonMutatingTest, MinElementWithPredicate) {
  std::list<int>::iterator i =
      abslx::c_min_element(sequence_, std::greater<int>());
  ASSERT_TRUE(i != sequence_.end());
  EXPECT_EQ(*i, 3);
}

TEST_F(NonMutatingTest, MaxElement) {
  std::list<int>::iterator i = abslx::c_max_element(sequence_);
  ASSERT_TRUE(i != sequence_.end());
  EXPECT_EQ(*i, 3);
}
...
TEST_F(NonMutatingTest, MaxElementWithPredicate) {
  std::list<int>::iterator i =
      abslx::c_max_element(sequence_, std::greater<int>());
  ASSERT_TRUE(i != sequence_.end());
  EXPECT_EQ(*i, 1);
}

TEST_F(NonMutatingTest, LexicographicalCompare) {
  EXPECT_FALSE(abslx::c_lexicographical_compare(sequence_, sequence_));
  ...
  EXPECT_TRUE(abslx::c_lexicographical_compare(sequence_, v));
  EXPECT_TRUE(abslx::c_lexicographical_compare(std::list<int>(sequence_), v));
}
...
TEST_F(NonMutatingTest, LexicographicalCopmareWithPredicate) {
  ...
  EXPECT_TRUE(
      abslx::c_lexicographical_compare(v, sequence_, std::greater<int>()));
  EXPECT_TRUE(abslx::c_lexicographical_compare(
...
TEST_F(NonMutatingTest, Includes) {
  ...
  s.insert(4);
  EXPECT_TRUE(abslx::c_includes(s, vector_));
}
...
TEST_F(NonMutatingTest, IncludesWithPredicate) {
  ...
  s.insert(4);
  EXPECT_TRUE(abslx::c_includes(s, v, std::greater<int>()));
}
...
TEST_F(NumericMutatingTest, Iota) {
  abslx::c_iota(list_, 5);
  std::list<int> expected{5, 6, 7};
  EXPECT_EQ(list_, expected);
}

TEST_F(NonMutatingTest, Accumulate) {
  EXPECT_EQ(abslx::c_accumulate(sequence_, 4), 1 + 2 + 3 + 4);
}

TEST_F(NonMutatingTest, AccumulateWithBinaryOp) {
  EXPECT_EQ(abslx::c_accumulate(sequence_, 4, std::multiplies<int>()),
            1 * 2 * 3 * 4);
}

TEST_F(NonMutatingTest, AccumulateLvalueInit) {
  int lvalue = 4;
  EXPECT_EQ(abslx::c_accumulate(sequence_, lvalue), 1 + 2 + 3 + 4);
}

TEST_F(NonMutatingTest, AccumulateWithBinaryOpLvalueInit) {
  int lvalue = 4;
  EXPECT_EQ(abslx::c_accumulate(sequence_, lvalue, std::multiplies<int>()),
            1 * 2 * 3 * 4);
}

TEST_F(NonMutatingTest, InnerProduct) {
  EXPECT_EQ(abslx::c_inner_product(sequence_, vector_, 1000),
            1000 + 1 * 1 + 2 * 2 + 3 * 3);
}

TEST_F(NonMutatingTest, InnerProductWithBinaryOps) {
  EXPECT_EQ(abslx::c_inner_product(sequence_, vector_, 10,
...
TEST_F(NonMutatingTest, InnerProductLvalueInit) {
  int lvalue = 1000;
  EXPECT_EQ(abslx::c_inner_product(sequence_, vector_, lvalue),
            1000 + 1 * 1 + 2 * 2 + 3 * 3);
}
...
TEST_F(NumericMutatingTest, AdjacentDifference) {
  auto last = abslx::c_adjacent_difference(list_, std::back_inserter(output_));
  *last = 1000;
...
TEST_F(NumericMutatingTest, PartialSum) {
  auto last = abslx::c_partial_sum(list_, std::back_inserter(output_));
  *last = 1000;
...
TEST_F(NonMutatingTest, LinearSearch) {
  EXPECT_TRUE(abslx::c_linear_search(container_, 3));
  EXPECT_FALSE(abslx::c_linear_search(container_, 4));
}

TEST_F(NonMutatingTest, AllOf) {
  const std::vector<int>& v = vector_;
  EXPECT_FALSE(abslx::c_all_of(v, [](int x) { return x > 1; }));
  EXPECT_TRUE(abslx::c_all_of(v, [](int x) { return x > 0; }));
}

TEST_F(NonMutatingTest, AnyOf) {
  const std::vector<int>& v = vector_;
  EXPECT_TRUE(abslx::c_any_of(v, [](int x) { return x > 2; }));
  EXPECT_FALSE(abslx::c_any_of(v, [](int x) { return x > 5; }));
}

TEST_F(NonMutatingTest, NoneOf) {
  const std::vector<int>& v = vector_;
  EXPECT_FALSE(abslx::c_none_of(v, [](int x) { return x > 2; }));
  EXPECT_TRUE(abslx::c_none_of(v, [](int x) { return x > 5; }));
}

TEST_F(NonMutatingTest, MinMaxElementLess) {
  std::pair<std::vector<int>::const_iterator, std::vector<int>::const_iterator>
      p = abslx::c_minmax_element(vector_, std::less<int>());
...
TEST_F(NonMutatingTest, MinMaxElementGreater) {
  std::pair<std::vector<int>::const_iterator, std::vector<int>::const_iterator>
      p = abslx::c_minmax_element(vector_, std::greater<int>());
...
TEST_F(NonMutatingTest, MinMaxElementNoPredicate) {
  std::pair<std::vector<int>::const_iterator, std::vector<int>::const_iterator>
      p = abslx::c_minmax_element(vector_);
  EXPECT_TRUE(p.first == vector_.begin());
  EXPECT_TRUE(p.second == vector_.begin() + 2);
}
...
TEST_F(SortingTest, IsSorted) {
  EXPECT_TRUE(abslx::c_is_sorted(sorted_));
  EXPECT_FALSE(abslx::c_is_sorted(unsorted_));
  EXPECT_FALSE(abslx::c_is_sorted(reversed_));
}

TEST_F(SortingTest, IsSortedWithPredicate) {
  EXPECT_FALSE(abslx::c_is_sorted(sorted_, std::greater<int>()));
  EXPECT_FALSE(abslx::c_is_sorted(unsorted_, std::greater<int>()));
  EXPECT_TRUE(abslx::c_is_sorted(reversed_, std::greater<int>()));
}

TEST_F(SortingTest, IsSortedUntil) {
  EXPECT_EQ(1, *abslx::c_is_sorted_until(unsorted_));
  EXPECT_EQ(4, *abslx::c_is_sorted_until(unsorted_, std::greater<int>()));
}

TEST_F(SortingTest, NthElement) {
  std::vector<int> unsorted = {2, 4, 1, 3};
  abslx::c_nth_element(unsorted, unsorted.begin() + 2);
  EXPECT_THAT(unsorted, ElementsAre(Lt(3), Lt(3), 3, Gt(3)));
  abslx::c_nth_element(unsorted, unsorted.begin() + 2, std::greater<int>());
  EXPECT_THAT(unsorted, ElementsAre(Gt(2), Gt(2), 2, Lt(2)));
}

TEST(MutatingTest, IsPartitioned) {
  EXPECT_TRUE(
      abslx::c_is_partitioned(std::vector<int>{1, 3, 5, 2, 4, 6}, IsOdd));
  EXPECT_FALSE(
      abslx::c_is_partitioned(std::vector<int>{1, 2, 3, 4, 5, 6}, IsOdd));
  EXPECT_FALSE(
      abslx::c_is_partitioned(std::vector<int>{2, 4, 6, 1, 3, 5}, IsOdd));
}

TEST(MutatingTest, Partition) {
  std::vector<int> actual = {1, 2, 3, 4, 5};
  abslx::c_partition(actual, IsOdd);
  EXPECT_THAT(actual, Truly([](const std::vector<int>& c) {
                return abslx::c_is_partitioned(c, IsOdd);
              }));
}

TEST(MutatingTest, StablePartition) {
  std::vector<int> actual = {1, 2, 3, 4, 5};
  abslx::c_stable_partition(actual, IsOdd);
...
TEST(MutatingTest, PartitionCopy) {
  ...
  auto ends = abslx::c_partition_copy(initial, back_inserter(odds),
...
TEST(MutatingTest, PartitionPoint) {
  ...
  auto middle = abslx::c_partition_point(initial, IsOdd);
  EXPECT_EQ(2, *middle);
}
...
TEST(MutatingTest, CopyFrontInserter) {
  ...
  abslx::c_copy(input, std::front_inserter(test_list));
...
TEST(MutatingTest, CopyBackInserter) {
  ...
  std::list<int> test_list(initial.begin(), initial.end());
  abslx::c_copy(input, std::back_inserter(test_list));
  EXPECT_EQ(std::list<int>(expected.begin(), expected.end()), test_list);

  std::vector<int> test_vector = initial;
  abslx::c_copy(input, std::back_inserter(test_vector));
  EXPECT_EQ(expected, test_vector);
}
...
TEST(MutatingTest, CopyN) {
  ...
  abslx::c_copy_n(initial, 2, back_inserter(actual));
...
TEST(MutatingTest, CopyIf) {
  ...
  abslx::c_copy_if(input, std::back_inserter(output),
                  [](int i) { return i != 2; });
...
TEST(MutatingTest, CopyBackward) {
  ...
  abslx::c_copy_backward(abslx::MakeSpan(actual.data(), 3), actual.end());
  EXPECT_EQ(expected, actual);
}

TEST(MutatingTest, Move) {
  std::vector<std::unique_ptr<int>> src;
  src.emplace_back(abslx::make_unique<int>(1));
  src.emplace_back(abslx::make_unique<int>(2));
  src.emplace_back(abslx::make_unique<int>(3));
  src.emplace_back(abslx::make_unique<int>(4));
  src.emplace_back(abslx::make_unique<int>(5));

  std::vector<std::unique_ptr<int>> dest = {};
  abslx::c_move(src, std::back_inserter(dest));
...
TEST(MutatingTest, MoveBackward) {
  ...
  actual.emplace_back(abslx::make_unique<int>(1));
  actual.emplace_back(abslx::make_unique<int>(2));
  actual.emplace_back(abslx::make_unique<int>(3));
  actual.emplace_back(abslx::make_unique<int>(4));
  actual.emplace_back(abslx::make_unique<int>(5));
  auto subrange = abslx::MakeSpan(actual.data(), 3);
  abslx::c_move_backward(subrange, actual.end());
...
TEST(MutatingTest, MoveWithRvalue) {
  ...
  abslx::c_move(MakeRValueSrc(), std::back_inserter(dest));
...
TEST(MutatingTest, SwapRanges) {
  ...
  abslx::c_swap_ranges(odds, evens);
  ...
  abslx::c_swap_ranges(evens, odds);
...
TEST_F(NonMutatingTest, Transform) {
  ...
  auto end = abslx::c_transform(x, back_inserter(y), std::negate<int>());
  EXPECT_EQ(std::vector<int>({0, -2, -4}), y);
  *end = 7;
  EXPECT_EQ(std::vector<int>({0, -2, -4, 7}), y);

  y = {1, 3, 0};
  end = abslx::c_transform(x, y, back_inserter(z), std::plus<int>());
  ...
  *end = 7;
  ...
  z.clear();
  y.pop_back();
  end = abslx::c_transform(x, y, std::back_inserter(z), std::plus<int>());
...
TEST(MutatingTest, Replace) {
  ...
  std::vector<int> test_vector = initial;
  abslx::c_replace(test_vector, 1, 4);
  EXPECT_EQ(expected, test_vector);

  std::list<int> test_list(initial.begin(), initial.end());
  abslx::c_replace(test_list, 1, 4);
  EXPECT_EQ(std::list<int>(expected.begin(), expected.end()), test_list);
}
...
TEST(MutatingTest, ReplaceIf) {
  ...
  abslx::c_replace_if(actual, IsOdd, 0);
  EXPECT_EQ(expected, actual);
}
...
TEST(MutatingTest, ReplaceCopy) {
  ...
  std::vector<int> actual;
  abslx::c_replace_copy(initial, back_inserter(actual), 1, 4);
  EXPECT_EQ(expected, actual);
}

TEST(MutatingTest, Sort) {
  std::vector<int> test_vector = {2, 3, 1, 4};
  abslx::c_sort(test_vector);
  EXPECT_THAT(test_vector, ElementsAre(1, 2, 3, 4));
}

TEST(MutatingTest, SortWithPredicate) {
  std::vector<int> test_vector = {2, 3, 1, 4};
  abslx::c_sort(test_vector, std::greater<int>());
  EXPECT_THAT(test_vector, ElementsAre(4, 3, 2, 1));
}

// For abslx::c_stable_sort tests. Needs an operator< that does not cover all
...
TEST(MutatingTest, StableSort) {
  ...
  abslx::c_stable_sort(test_vector);
...
TEST(MutatingTest, StableSortWithPredicate) {
  std::vector<Element> test_vector = {{1, 1}, {2, 1}, {2, 0}, {1, 0}, {2, 2}};
  abslx::c_stable_sort(test_vector, [](const Element& e1, const Element& e2) {
    return e2 < e1;
  });
  EXPECT_THAT(test_vector,
...
TEST(MutatingTest, Generate) {
  std::vector<int> actual(5);
  int x = 0;
  abslx::c_generate(actual, [&x]() { return ++x; });
...
TEST(MutatingTest, GenerateN) {
  std::vector<int> actual(5, 0);
  int x = 0;
  abslx::c_generate_n(actual, 3, [&x]() { return ++x; });
  EXPECT_THAT(actual, ElementsAre(1, 2, 3, 0, 0));
}

TEST(MutatingTest, RemoveCopy) {
  std::vector<int> actual;
  abslx::c_remove_copy(std::vector<int>{1, 2, 3}, back_inserter(actual), 2);
  EXPECT_THAT(actual, ElementsAre(1, 3));
}

TEST(MutatingTest, RemoveCopyIf) {
  std::vector<int> actual;
  abslx::c_remove_copy_if(std::vector<int>{1, 2, 3}, back_inserter(actual),
                         IsOdd);
...
TEST(MutatingTest, UniqueCopy) {
  std::vector<int> actual;
  abslx::c_unique_copy(std::vector<int>{1, 2, 2, 2, 3, 3, 2},
...
TEST(MutatingTest, UniqueCopyWithPredicate) {
  std::vector<int> actual;
  abslx::c_unique_copy(std::vector<int>{1, 2, 3, -1, -2, -3, 1},
                      back_inserter(actual),
...
TEST(MutatingTest, RotateCopy) {
  ...
  std::vector<int> actual;
  auto end =
      abslx::c_rotate_copy(initial, initial.begin() + 2, back_inserter(actual));
  *end = 5;
...
TEST(MutatingTest, Shuffle) {
  std::vector<int> actual = {1, 2, 3, 4, 5};
  abslx::c_shuffle(actual, std::random_device());
  EXPECT_THAT(actual, UnorderedElementsAre(1, 2, 3, 4, 5));
}

TEST(MutatingTest, PartialSort) {
  std::vector<int> sequence{5, 3, 42, 0};
  abslx::c_partial_sort(sequence, sequence.begin() + 2);
  EXPECT_THAT(abslx::MakeSpan(sequence.data(), 2), ElementsAre(0, 3));
  abslx::c_partial_sort(sequence, sequence.begin() + 2, std::greater<int>());
  EXPECT_THAT(abslx::MakeSpan(sequence.data(), 2), ElementsAre(42, 5));
}

TEST(MutatingTest, PartialSortCopy) {
  const std::vector<int> initial = {5, 3, 42, 0};
  std::vector<int> actual(2);
  abslx::c_partial_sort_copy(initial, actual);
  EXPECT_THAT(actual, ElementsAre(0, 3));
  abslx::c_partial_sort_copy(initial, actual, std::greater<int>());
  EXPECT_THAT(actual, ElementsAre(42, 5));
}

TEST(MutatingTest, Merge) {
  std::vector<int> actual;
  abslx::c_merge(std::vector<int>{1, 3, 5}, std::vector<int>{2, 4},
                back_inserter(actual));
  EXPECT_THAT(actual, ElementsAre(1, 2, 3, 4, 5));
}
...
TEST(MutatingTest, MergeWithComparator) {
  std::vector<int> actual;
  abslx::c_merge(std::vector<int>{5, 3, 1}, std::vector<int>{4, 2},
...
TEST(MutatingTest, InplaceMerge) {
  ...
  abslx::c_inplace_merge(actual, actual.begin() + 3);
  EXPECT_THAT(actual, ElementsAre(1, 2, 3, 4, 5));
}

TEST(MutatingTest, InplaceMergeWithComparator) {
  std::vector<int> actual = {5, 3, 1, 4, 2};
  abslx::c_inplace_merge(actual, actual.begin() + 3, std::greater<int>());
  EXPECT_THAT(actual, ElementsAre(5, 4, 3, 2, 1));
}
...
TEST_F(SetOperationsTest, SetUnion) {
  std::vector<int> actual;
  abslx::c_set_union(a_, b_, back_inserter(actual));
  EXPECT_THAT(actual, ElementsAre(1, 2, 3, 5));
}

TEST_F(SetOperationsTest, SetUnionWithComparator) {
  std::vector<int> actual;
  abslx::c_set_union(a_reversed_, b_reversed_, back_inserter(actual),
                    std::greater<int>());
...
TEST_F(SetOperationsTest, SetIntersection) {
  std::vector<int> actual;
  abslx::c_set_intersection(a_, b_, back_inserter(actual));
  EXPECT_THAT(actual, ElementsAre(1, 3));
}

TEST_F(SetOperationsTest, SetIntersectionWithComparator) {
  std::vector<int> actual;
  abslx::c_set_intersection(a_reversed_, b_reversed_, back_inserter(actual),
                           std::greater<int>());
...
TEST_F(SetOperationsTest, SetDifference) {
  std::vector<int> actual;
  abslx::c_set_difference(a_, b_, back_inserter(actual));
  EXPECT_THAT(actual, ElementsAre(2));
}

TEST_F(SetOperationsTest, SetDifferenceWithComparator) {
  std::vector<int> actual;
  abslx::c_set_difference(a_reversed_, b_reversed_, back_inserter(actual),
                         std::greater<int>());
  EXPECT_THAT(actual, ElementsAre(2));
}
...
TEST_F(SetOperationsTest, SetSymmetricDifference) {
  std::vector<int> actual;
  abslx::c_set_symmetric_difference(a_, b_, back_inserter(actual));
  EXPECT_THAT(actual, ElementsAre(2, 5));
}

TEST_F(SetOperationsTest, SetSymmetricDifferenceWithComparator) {
  std::vector<int> actual;
  abslx::c_set_symmetric_difference(a_reversed_, b_reversed_,
                                   back_inserter(actual), std::greater<int>());
...
TEST(HeapOperationsTest, WithoutComparator) {
  ...
  EXPECT_FALSE(abslx::c_is_heap(heap));
  abslx::c_make_heap(heap);
  EXPECT_TRUE(abslx::c_is_heap(heap));
  heap.push_back(4);
  EXPECT_EQ(3, abslx::c_is_heap_until(heap) - heap.begin());
  abslx::c_push_heap(heap);
  EXPECT_EQ(4, heap[0]);
  abslx::c_pop_heap(heap);
  EXPECT_EQ(4, heap[3]);
  abslx::c_make_heap(heap);
  abslx::c_sort_heap(heap);
...
TEST(HeapOperationsTest, WithComparator) {
  ...
  EXPECT_FALSE(abslx::c_is_heap(heap, greater()));
  abslx::c_make_heap(heap, greater());
  EXPECT_TRUE(abslx::c_is_heap(heap, greater()));
  heap.push_back(0);
  EXPECT_EQ(3, abslx::c_is_heap_until(heap, greater()) - heap.begin());
  abslx::c_push_heap(heap, greater());
  EXPECT_EQ(0, heap[0]);
  abslx::c_pop_heap(heap, greater());
  EXPECT_EQ(0, heap[3]);
  abslx::c_make_heap(heap, greater());
  abslx::c_sort_heap(heap, greater());
...
TEST(MutatingTest, PermutationOperations) {
  ...
  abslx::c_next_permutation(permuted);
  EXPECT_TRUE(abslx::c_is_permutation(initial, permuted));
  EXPECT_TRUE(abslx::c_is_permutation(initial, permuted, std::equal_to<int>()));

  std::vector<int> permuted2 = initial;
  abslx::c_prev_permutation(permuted2, std::greater<int>());
  EXPECT_EQ(permuted, permuted2);

  abslx::c_prev_permutation(permuted);
