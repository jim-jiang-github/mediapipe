...
// stored. (Note that the frame generated for the `abslx::GetStackFrames()`
...
//        void* result[10];
//        int sizes[10];
//        int depth = abslx::GetStackFrames(result, sizes, 10, 1);
//      }
//
...
// that the frame generated for the `abslx::GetStackFramesWithContext()` routine
...
// stored. Note that this function is similar to `abslx::GetStackFrames()`
...
//      bar() {
//        void* result[10];
//        int depth = abslx::GetStackTrace(result, 10, 1);
//      }
//
// This produces:
//
...
// the `abslx::GetStackFramesWithContext()` routine itself is also skipped.)
//
// The `uc` parameter, if non-null, should be a pointer to a `ucontext_t` value
// passed to a signal handler registered via the `sa_sigaction` field of a
// `sigaction` struct. (See
...
// arguments passed to `abslx::GetStackFramesWithContext()` except that sizes
...
// *****************************************************************************
//
// abslx::SetStackUnwinder is not suitable for general purpose use.  It is
// provided for custom runtimes.
// Some things to watch out for when calling `abslx::SetStackUnwinder()`:
//
...
// case where `abslx::SetStackUnwinder()` was never called. Equivalents are:
//
