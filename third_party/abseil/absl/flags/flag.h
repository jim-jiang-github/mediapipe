...
// -----------------------------------------------------------------------------
//
// This header file defines the `abslx::Flag<T>` type for holding command-line
...
// detail) and you do not construct or manipulate actual `abslx::Flag<T>`
...
// using the `abslx::GetFlag()` and `abslx::SetFlag()` functions.
...
// Flag
//
// An `abslx::Flag` holds a command-line flag value, providing a runtime
...
//
// You should not construct and cannot use the `abslx::Flag` type directly;
...
//
// No public methods of `abslx::Flag<T>` are part of the Abseil Flags API.
...
// class `abslx::Flag` contains an `internal::Flag*` (instead of being an alias
// to that class) and dynamically allocates an instance when necessary. We also
// forward all calls to internal::Flag methods via trampoline methods. In this
// setup the `abslx::Flag` class does not have constructor and virtual methods,
...
abslx::Mutex* GetGlobalConstructionGuard();
}  // namespace flags_internal
 ...
 public:
  ...
  flags_internal::Flag<T>& GetImpl() const {
    if (!inited_.load(std::memory_order_acquire)) {
      abslx::MutexLock l(flags_internal::GetGlobalConstructionGuard());
  ...
  // Public methods of `abslx::Flag<T>` are NOT part of the Abseil Flags API.
  // See https://abseil.io/docs/cpp/guides/flags
  bool IsRetired() const { return GetImpl().IsRetired(); }
  abslx::string_view Name() const { return GetImpl().Name(); }
...
// GetFlag()
//
// Returns the value (of type `T`) of an `abslx::Flag<T>` instance, by value. Do
// not construct an `abslx::Flag<T>` directly and call `abslx::GetFlag()`;
// instead, refer to flag's constructed variable name (e.g. `FLAGS_name`).
// Because this function returns by value and not by reference, it is
// thread-safe, but note that the operation may be expensive; as a result, avoid
// `abslx::GetFlag()` within any tight loops.
//
// Example:
//
//   // FLAGS_count is a Flag of type `int`
//   int my_count = abslx::GetFlag(FLAGS_count);
//
//   // FLAGS_firstname is a Flag of type `std::string`
//   std::string first_name = abslx::GetFlag(FLAGS_firstname);
template <typename T>
ABSL_MUST_USE_RESULT T GetFlag(const abslx::Flag<T>& flag) {
  return flags_internal::FlagImplPeer::InvokeGet<T>(flag);
}

// SetFlag()
//
// Sets the value of an `abslx::Flag` to the value `v`. Do not construct an
// `abslx::Flag<T>` directly and call `abslx::SetFlag()`; instead, use the
// flag's variable name (e.g. `FLAGS_name`). This function is
// thread-safe, but is potentially expensive. Avoid setting flags in general,
// but especially within performance-critical code.
template <typename T>
void SetFlag(abslx::Flag<T>* flag, const T& v) {
...
void SetFlag(abslx::Flag<T>* flag, const V& v) {
  T value(v);
...
// Example:
//
//   std::string = abslx::GetFlagReflectionHandle(FLAGS_count).DefaultValue();

template <typename T>
const CommandLineFlag& GetFlagReflectionHandle(const abslx::Flag<T>& f) {
...
// ABSL_FLAG()
//
// This macro defines an `abslx::Flag<T>` instance of a specified type `T`:
//
...
//
// Note: do not construct objects of type `abslx::Flag<T>` directly. Only use the
...
#define ABSL_FLAG_IMPL_HELP_ARG(name)                      \
  abslx::flags_internal::HelpArg<AbslFlagHelpGenFor##name>( \
      FLAGS_help_storage_##name)
#define ABSL_FLAG_IMPL_DEFAULT_ARG(Type, name) \
  abslx::flags_internal::DefaultArg<Type, AbslFlagDefaultGenFor##name>(0)
#else
...
#define ABSL_FLAG_IMPL_REGISTRAR(T, flag)                                      \
  abslx::flags_internal::FlagRegistrar<T, false>(ABSL_FLAG_IMPL_FLAG_PTR(flag), \
                                                nullptr)
#else
#define ABSL_FLAG_IMPL_FLAGNAME(txt) txt
#define ABSL_FLAG_IMPL_FILENAME() __FILE__
#define ABSL_FLAG_IMPL_REGISTRAR(T, flag)                                     \
  abslx::flags_internal::FlagRegistrar<T, true>(ABSL_FLAG_IMPL_FLAG_PTR(flag), \
                                               __FILE__)
#endif
...
#define ABSL_FLAG_IMPL_FLAGHELP(txt) abslx::flags_internal::kStrippedFlagHelp
#else
...
// between the two via the call to HelpArg in abslx::Flag instantiation below.
...
#define ABSL_FLAG_IMPL_DECLARE_HELP_WRAPPER(name, txt)                       \
  struct AbslFlagHelpGenFor##name {                                          \
    ...
    static constexpr abslx::string_view Value(                                \
        abslx::string_view v = ABSL_FLAG_IMPL_FLAGHELP(txt)) {                \
      return v;                                                              \
    }                                                                        \
  ...
  constexpr auto FLAGS_help_storage_##name ABSL_INTERNAL_UNIQUE_SMALL_NAME() \
      ABSL_ATTRIBUTE_SECTION_VARIABLE(flags_help_cold) =                     \
          abslx::flags_internal::HelpStringAsArray<AbslFlagHelpGenFor##name>( \
              0);

#define ABSL_FLAG_IMPL_DECLARE_DEF_VAL_WRAPPER(name, Type, default_value)     \
  struct AbslFlagDefaultGenFor##name {                                        \
    Type value = abslx::flags_internal::InitDefaultValue<Type>(default_value); \
...
#define ABSL_FLAG_IMPL(Type, name, default_value, help)                       \
  namespace abslx /* block flags in namespaces */ {}                           \
  ABSL_FLAG_IMPL_DECLARE_DEF_VAL_WRAPPER(name, Type, default_value)           \
  ABSL_FLAG_IMPL_DECLARE_HELP_WRAPPER(name, help)                             \
  ABSL_CONST_INIT abslx::Flag<Type> FLAGS_##name{                              \
      ABSL_FLAG_IMPL_FLAGNAME(#name), ABSL_FLAG_IMPL_FILENAME(),              \
      ABSL_FLAG_IMPL_HELP_ARG(name), ABSL_FLAG_IMPL_DEFAULT_ARG(Type, name)}; \
  extern abslx::flags_internal::FlagRegistrarEmpty FLAGS_no##name;             \
  abslx::flags_internal::FlagRegistrarEmpty FLAGS_no##name =                   \
...
#define ABSL_RETIRED_FLAG(type, name, default_value, explanation)      \
  static abslx::flags_internal::RetiredFlag<type> RETIRED_FLAGS_##name; \
  ABSL_ATTRIBUTE_UNUSED static const auto RETIRED_FLAGS_REG_##name =   \
      (RETIRED_FLAGS_##name.Retire(#name),                             \
       ::abslx::flags_internal::FlagRegistrarEmpty{})
