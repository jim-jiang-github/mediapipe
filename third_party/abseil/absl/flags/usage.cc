...
namespace {
ABSL_CONST_INIT abslx::Mutex usage_message_guard(abslx::kConstInit);
...
void SetProgramUsageMessage(abslx::string_view new_usage_message) {
  abslx::MutexLock l(&flags_internal::usage_message_guard);
...
abslx::string_view ProgramUsageMessage() {
  abslx::MutexLock l(&flags_internal::usage_message_guard);

  return flags_internal::program_usage_message != nullptr
             ? abslx::string_view(*flags_internal::program_usage_message)
