...
TEST(BindTest, Basics) {
  EXPECT_EQ('C', abslx::bind_front(CharAt)("ABC", 2));
  EXPECT_EQ('C', abslx::bind_front(CharAt, "ABC")(2));
  EXPECT_EQ('C', abslx::bind_front(CharAt, "ABC", 2)());
}
...
TEST(BindTest, Lambda) {
  ...
  EXPECT_EQ(6, abslx::bind_front(lambda)(1, 2, 3));
  EXPECT_EQ(6, abslx::bind_front(lambda, 1)(2, 3));
  EXPECT_EQ(6, abslx::bind_front(lambda, 1, 2)(3));
  EXPECT_EQ(6, abslx::bind_front(lambda, 1, 2, 3)());
}
...
TEST(BindTest, PerfectForwardingOfBoundArgs) {
  auto f = abslx::bind_front(Functor());
  const auto& cf = f;
  EXPECT_EQ("&", f());
  EXPECT_EQ("const&", cf());
...
TEST(BindTest, PerfectForwardingOfFreeArgs) {
  ArgDescribe f;
  int i;
  EXPECT_EQ("&", abslx::bind_front(f)(static_cast<int&>(i)));
  EXPECT_EQ("const&", abslx::bind_front(f)(static_cast<const int&>(i)));
  EXPECT_EQ("&&", abslx::bind_front(f)(static_cast<int&&>(i)));
  EXPECT_EQ("const&&", abslx::bind_front(f)(static_cast<const int&&>(i)));
}
...
TEST(BindTest, RefToFunctor) {
  ...
  auto bound_ncf = abslx::bind_front(std::ref(ncf));
...
TEST(BindTest, StoreByCopy) {
  Struct s = {"hello"};
  auto f = abslx::bind_front(&Struct::value, s);
  auto g = f;
  EXPECT_EQ("hello", f());
  EXPECT_EQ("hello", g());
  EXPECT_NE(&s.value, &f());
...
TEST(BindTest, StoreByRef) {
  NonCopyable s("hello");
  auto f = abslx::bind_front(&GetNonCopyableValue, std::ref(s));
  EXPECT_EQ("hello", f());
  EXPECT_EQ(&s.value, &f());
...
TEST(BindTest, StoreByCRef) {
  NonCopyable s("hello");
  auto f = abslx::bind_front(&GetNonCopyableValue, std::cref(s));
  EXPECT_EQ("hello", f());
  EXPECT_EQ(&s.value, &f());
  auto g = std::move(f);  // NOLINT
  EXPECT_EQ("hello", g());
...
TEST(BindTest, StoreByRefInvokeByWrapper) {
  NonCopyable s("hello");
  auto f = abslx::bind_front(GetNonCopyableValueByWrapper, std::ref(s));
  EXPECT_EQ("hello", f());
  EXPECT_EQ(&s.value, &f());
  auto g = std::move(f);
  EXPECT_EQ("hello", g());
...
TEST(BindTest, StoreByPointer) {
  NonCopyable s("hello");
  auto f = abslx::bind_front(&NonCopyable::value, &s);
  EXPECT_EQ("hello", f());
  EXPECT_EQ(&s.value, &f());
  auto g = std::move(f);
  EXPECT_EQ("hello", g());
...
TEST(BindTest, NonCopyableArg) {
  EXPECT_EQ(42, abslx::bind_front(Sink)(abslx::make_unique<int>(42)));
  EXPECT_EQ(42, abslx::bind_front(Sink, abslx::make_unique<int>(42))());
}

TEST(BindTest, NonCopyableResult) {
  EXPECT_THAT(abslx::bind_front(Factory)(42), ::testing::Pointee(42));
  EXPECT_THAT(abslx::bind_front(Factory, 42)(), ::testing::Pointee(42));
}
...
TEST(BindTest, WrappedMoveOnly) {
  ...
  x.m = abslx::make_unique<int>(42);
  auto f = abslx::bind_front(&GetMember, std::move(x));
...
TEST(BindTest, ConstExpr) {
  constexpr auto f = abslx::bind_front(CharAt);
  EXPECT_EQ(f("ABC", 1), 'B');
  static constexpr int five = 5;
  constexpr auto plus5 = abslx::bind_front(Plus, five);
  EXPECT_EQ(plus5(1), 6);
  ...
  constexpr auto g = abslx::bind_front(CharAt, data);
  EXPECT_EQ(g(1), 'E');
#endif
}
...
TEST(BindTest, Mangling) {
  ...
  abslx::bind_front(ManglingCall{}, 1, 3.3)("A");
}
