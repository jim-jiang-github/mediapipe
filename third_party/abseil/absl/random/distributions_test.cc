...
auto InferredUniformReturnT(int)
    -> decltype(abslx::Uniform(std::declval<abslx::InsecureBitGen&>(),
...
auto InferredTaggedUniformReturnT(int)
    -> decltype(abslx::Uniform(std::declval<TagType>(),
                              std::declval<abslx::InsecureBitGen&>(),
                              std::declval<A>(), std::declval<B>()));
...
//
//   abslx::Uniform(gen, A{}, B{})
//
// returns the type "Expect".
//
// This interface can also be used to assert that a given abslx::Uniform()
// overload does not exist / will not compile. Given types <A, B>, the
// expression
//
//   decltype(abslx::Uniform(..., std::declval<A>(), std::declval<B>()))
//
...
//
//   abslx::Uniform(gen, 1.23f, std::numeric_limits<int>::max() - 1)
//
...
auto ExplicitTaggedUniformReturnT(int) -> decltype(abslx::Uniform<ExplicitRet>(
    std::declval<TagType>(), *std::declval<abslx::InsecureBitGen*>(),
    std::declval<A>(), std::declval<B>()));
      ...
      abslx::conjunction<
          std::is_same<Expect,
                       decltype(ExplicitTaggedUniformReturnT<
                                abslx::IntervalOpenOpenTag, A, B, Expect>(0))>,
          std::is_same<Expect, decltype(ExplicitTaggedUniformReturnT<
                                        abslx::IntervalOpenOpenTag, B, A,
                                        Expect>(0))>>::value,
      "");
}
...
TEST_F(RandomDistributionsTest, UniformExamples) {
  ...
  EXPECT_NE(1, abslx::Uniform(gen, static_cast<uint16_t>(0), 1.0f));
  EXPECT_NE(1, abslx::Uniform(gen, 0, 1.0));
  EXPECT_NE(1, abslx::Uniform(abslx::IntervalOpenOpen, gen,
                             static_cast<uint16_t>(0), 1.0f));
  EXPECT_NE(1, abslx::Uniform(abslx::IntervalOpenOpen, gen, 0, 1.0));
  EXPECT_NE(1, abslx::Uniform(abslx::IntervalOpenOpen, gen, -1, 1.0));
  EXPECT_NE(1, abslx::Uniform<double>(abslx::IntervalOpenOpen, gen, -1, 1));
  EXPECT_NE(1, abslx::Uniform<float>(abslx::IntervalOpenOpen, gen, 0, 1));
  EXPECT_NE(1, abslx::Uniform<float>(gen, 0, 1));
}

TEST_F(RandomDistributionsTest, UniformNoBounds) {
  abslx::InsecureBitGen gen;

  abslx::Uniform<uint8_t>(gen);
  abslx::Uniform<uint16_t>(gen);
  abslx::Uniform<uint32_t>(gen);
  abslx::Uniform<uint64_t>(gen);
}
...
TEST_F(RandomDistributionsTest, UniformNonsenseRanges) {
  ...
  // <uint>
  EXPECT_EQ(0, abslx::Uniform<uint64_t>(gen, 0, 0));
  EXPECT_EQ(1, abslx::Uniform<uint64_t>(gen, 1, 0));
  EXPECT_EQ(0, abslx::Uniform<uint64_t>(abslx::IntervalOpenOpen, gen, 0, 0));
  EXPECT_EQ(1, abslx::Uniform<uint64_t>(abslx::IntervalOpenOpen, gen, 1, 0));

  constexpr auto m = (std::numeric_limits<uint64_t>::max)();

  EXPECT_EQ(m, abslx::Uniform(gen, m, m));
  EXPECT_EQ(m, abslx::Uniform(gen, m, m - 1));
  EXPECT_EQ(m - 1, abslx::Uniform(gen, m - 1, m));
  EXPECT_EQ(m, abslx::Uniform(abslx::IntervalOpenOpen, gen, m, m));
  EXPECT_EQ(m, abslx::Uniform(abslx::IntervalOpenOpen, gen, m, m - 1));
  EXPECT_EQ(m - 1, abslx::Uniform(abslx::IntervalOpenOpen, gen, m - 1, m));

  // <int>
  EXPECT_EQ(0, abslx::Uniform<int64_t>(gen, 0, 0));
  EXPECT_EQ(1, abslx::Uniform<int64_t>(gen, 1, 0));
  EXPECT_EQ(0, abslx::Uniform<int64_t>(abslx::IntervalOpenOpen, gen, 0, 0));
  EXPECT_EQ(1, abslx::Uniform<int64_t>(abslx::IntervalOpenOpen, gen, 1, 0));

  constexpr auto l = (std::numeric_limits<int64_t>::min)();
  constexpr auto r = (std::numeric_limits<int64_t>::max)();

  EXPECT_EQ(l, abslx::Uniform(gen, l, l));
  EXPECT_EQ(r, abslx::Uniform(gen, r, r));
  EXPECT_EQ(r, abslx::Uniform(gen, r, r - 1));
  EXPECT_EQ(r - 1, abslx::Uniform(gen, r - 1, r));
  EXPECT_EQ(l, abslx::Uniform(abslx::IntervalOpenOpen, gen, l, l));
  EXPECT_EQ(r, abslx::Uniform(abslx::IntervalOpenOpen, gen, r, r));
  EXPECT_EQ(r, abslx::Uniform(abslx::IntervalOpenOpen, gen, r, r - 1));
  EXPECT_EQ(r - 1, abslx::Uniform(abslx::IntervalOpenOpen, gen, r - 1, r));
  ...
  EXPECT_EQ(1.0, abslx::Uniform(gen, 1.0, e));
  EXPECT_EQ(1.0, abslx::Uniform(gen, 1.0, f));
  EXPECT_EQ(0.0, abslx::Uniform(gen, 0.0, g));

  EXPECT_EQ(e, abslx::Uniform(abslx::IntervalOpenOpen, gen, 1.0, e));
  EXPECT_EQ(f, abslx::Uniform(abslx::IntervalOpenOpen, gen, 1.0, f));
  EXPECT_EQ(g, abslx::Uniform(abslx::IntervalOpenOpen, gen, 0.0, g));
}
...
TEST_F(RandomDistributionsTest, UniformReal) {
  ...
  for (int i = 0; i < kSize; i++) {
    values[i] = abslx::Uniform(gen, 0, 1.0);
  }
...
TEST_F(RandomDistributionsTest, UniformInt) {
  ...
  for (int i = 0; i < kSize; i++) {
    ...
    int64_t j = abslx::Uniform(abslx::IntervalClosedClosed, gen, 0, kMax);
    // convert to double.
  ...
  /*
  // NOTE: These are not supported by abslx::Uniform, which is specialized
  ...
  abslx::Uniform(gen, E0, E1);
  abslx::Uniform(gen, S0, S1);
  abslx::Uniform(gen, U0, U1);
  */
}
...
TEST_F(RandomDistributionsTest, Exponential) {
  ...
  abslx::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = abslx::Exponential<double>(gen);
  }
...
TEST_F(RandomDistributionsTest, PoissonDefault) {
  ...
  abslx::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = abslx::Poisson<int64_t>(gen);
  }
...
TEST_F(RandomDistributionsTest, PoissonLarge) {
  ...
  abslx::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = abslx::Poisson<int64_t>(gen, kMean);
  }
...
TEST_F(RandomDistributionsTest, Bernoulli) {
  ...
  abslx::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = abslx::Bernoulli(gen, kP);
  }
...
TEST_F(RandomDistributionsTest, Beta) {
  ...
  abslx::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = abslx::Beta(gen, kAlpha, kBeta);
  }
...
TEST_F(RandomDistributionsTest, Zipf) {
  ...
  abslx::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = abslx::Zipf<int64_t>(gen, 100);
  }
...
TEST_F(RandomDistributionsTest, Gaussian) {
  ...
  abslx::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = abslx::Gaussian<double>(gen);
  }
...
TEST_F(RandomDistributionsTest, LogUniform) {
  ...
  abslx::InsecureBitGen gen;
  for (int i = 0; i < kSize; i++) {
    values[i] = abslx::LogUniform<int64_t>(gen, 0, (1 << 10) - 1);
  }
