...
// (they use abslx::ascii_tolower instead of tolower).
//
...
#include "absl/strings/ascii.h"  // for abslx::ascii_tolower
...
const char* int_memmatch(const char* haystack, size_t haylen,
  ...
  for (; haystack < hayend; ++haystack) {
    char hay = case_sensitive
                   ? *haystack
                   : abslx::ascii_tolower(static_cast<unsigned char>(*haystack));
    char nee = case_sensitive
                   ? *needle
                   : abslx::ascii_tolower(static_cast<unsigned char>(*needle));
    if (hay == nee) {
      if (++needle == needleend) {
