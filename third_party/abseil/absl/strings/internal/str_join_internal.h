...
class PairFormatterImpl {
 public:
  PairFormatterImpl(F1 f1, abslx::string_view sep, F2 f2)
...
template <>
struct DefaultFormatter<abslx::string_view> {
  typedef NoFormatter Type;
};
template <typename ValueType>
...
std::string JoinAlgorithm(Iterator start, Iterator end, abslx::string_view s,
                          Formatter&& f) {
  std::string result;
...
// std::vector<abslx::string_view>.
//
// This is an overload of the previous JoinAlgorithm() function. Here the
// Formatter argument is of type NoFormatter. Since NoFormatter is an internal
// type, this overload is only invoked when strings::Join() is called with a
// range of string-like objects (e.g., std::string, abslx::string_view), and an
...
struct JoinTupleLoop {
  ...
  void operator()(std::string* out, const Tup& tup, abslx::string_view sep,
                  Formatter&& fmt) {
...
struct JoinTupleLoop<N, N> {
  template <typename Tup, typename Formatter>
  void operator()(std::string*, const Tup&, abslx::string_view, Formatter&&) {}
};

template <typename... T, typename Formatter>
std::string JoinAlgorithm(const std::tuple<T...>& tup, abslx::string_view sep,
                          Formatter&& fmt) {
  std::string result;
...
std::string JoinRange(const Range& range, abslx::string_view separator,
                      Formatter&& fmt) {
  using std::begin;
  using std::end;
