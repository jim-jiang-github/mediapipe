...
TEST(StrJoin, APIExamples) {
  {
    ...
    EXPECT_EQ("foo-bar-baz", abslx::StrJoin(v, "-"));
  }

  {
    // Collection of abslx::string_view
    std::vector<abslx::string_view> v = {"foo", "bar", "baz"};
    EXPECT_EQ("foo-bar-baz", abslx::StrJoin(v, "-"));
  }
...

  {
    ...
    EXPECT_EQ("1-2-3--4", abslx::StrJoin(v, "-"));
  }

  {
    // Literals passed as a std::initializer_list
    std::string s = abslx::StrJoin({"a", "b", "c"}, "-");
    EXPECT_EQ("a-b-c", s);
  }
  {
    // Join a std::tuple<T...>.
    std::string s = abslx::StrJoin(std::make_tuple(123, "abc", 0.456), "-");
...

  {
    ...
    EXPECT_EQ("1-2-3", abslx::StrJoin(v, "-"));
  }

  {
    // Array of ints
    const int a[] = {1, 2, 3, -4};
    EXPECT_EQ("1-2-3--4", abslx::StrJoin(a, a + ABSL_ARRAYSIZE(a), "-"));
  }
...

  {
    ...
    EXPECT_EQ("a-b", abslx::StrJoin(v, "-"));
  }

  {
    // A std::map, which is a collection of std::pair<>s.
    std::map<std::string, int> m = {{"a", 1}, {"b", 2}, {"c", 3}};
    EXPECT_EQ("a=1,b=2,c=3", abslx::StrJoin(m, ",", abslx::PairFormatter("=")));
  }

  {
    // Shows abslx::StrSplit and abslx::StrJoin working together. This example is
    // equivalent to s/=/-/g.
    const std::string s = "a=b=c=d";
    EXPECT_EQ("a-b-c-d", abslx::StrJoin(abslx::StrSplit(s, "="), "-"));
  }
...

  {
    ...
    EXPECT_EQ("1-0-1", abslx::StrJoin(v, "-"));
  }
}
...
TEST(StrJoin, CustomFormatter) {
  ...
  {
    std::string joined =
        abslx::StrJoin(v, "", [](std::string* out, const std::string& in) {
          abslx::StrAppend(out, "(", in, ")");
        });
  ...
  }
  {
    ...
    EXPECT_EQ("(One)(Two)(Three)", abslx::StrJoin(v, "", ImmovableFormatter()));
  }
  {
    ...
    EXPECT_EQ("(One)(Two)(Three)", abslx::StrJoin(v, "", OverloadedFormatter()));
    const OverloadedFormatter fmt = {};
    EXPECT_EQ("[One][Two][Three]", abslx::StrJoin(v, "", fmt));
  }
}
...
TEST(AlphaNumFormatter, FormatterAPI) {
  ...
  auto f = abslx::AlphaNumFormatter();
  std::string s;
  f(&s, "Testing: ");
...
TEST(PairFormatter, FormatterAPI) {
  {
    ...
    const auto f = abslx::PairFormatter("=");
    std::string s;
...

  {
    ...
    auto f = abslx::PairFormatter(TestingParenFormatter(), "=",
...
TEST(DereferenceFormatter, FormatterAPI) {
  {
    ...
    const abslx::strings_internal::DereferenceFormatterImpl<
        abslx::strings_internal::AlphaNumFormatterImpl>
        f;
    int x = 1, y = 2, z = 3;
    std::string s;
    f(&s, &x);
...

  {
    ...
    abslx::strings_internal::DereferenceFormatterImpl<
        abslx::strings_internal::DefaultFormatter<std::string>::Type>
        f;
...

  {
    ...
    auto f = abslx::DereferenceFormatter(TestingParenFormatter());
    int x = 1, y = 2, z = 3;
    std::string s;
    f(&s, &x);
    f(&s, &y);
...
TEST(StrJoin, PublicAPIOverloads) {
  ...
  // Iterators + formatter
  EXPECT_EQ("a-b-c",
            abslx::StrJoin(v.begin(), v.end(), "-", abslx::AlphaNumFormatter()));
  // Range + formatter
  EXPECT_EQ("a-b-c", abslx::StrJoin(v, "-", abslx::AlphaNumFormatter()));
  // Iterators, no formatter
  EXPECT_EQ("a-b-c", abslx::StrJoin(v.begin(), v.end(), "-"));
  // Range, no formatter
  EXPECT_EQ("a-b-c", abslx::StrJoin(v, "-"));
}

TEST(StrJoin, Array) {
  const abslx::string_view a[] = {"a", "b", "c"};
  EXPECT_EQ("a-b-c", abslx::StrJoin(a, "-"));
}

TEST(StrJoin, InitializerList) {
  { EXPECT_EQ("a-b-c", abslx::StrJoin({"a", "b", "c"}, "-")); }
  ...
  {
    std::initializer_list<abslx::string_view> a = {"a", "b", "c"};
    EXPECT_EQ("a-b-c", abslx::StrJoin(a, "-"));
  }

  {
    ...
    EXPECT_EQ("(a)-(b)-(c)", abslx::StrJoin(a, "-", f));
  }

  {
    // initializer_list of ints
    EXPECT_EQ("1-2-3", abslx::StrJoin({1, 2, 3}, "-"));
  }

  {
    ...
    auto a = {1, 2, 3};
    TestingParenFormatter f;
    EXPECT_EQ("(1)-(2)-(3)", abslx::StrJoin(a, "-", f));
  }
}

TEST(StrJoin, Tuple) {
  EXPECT_EQ("", abslx::StrJoin(std::make_tuple(), "-"));
  EXPECT_EQ("hello", abslx::StrJoin(std::make_tuple("hello"), "-"));

  int x(10);
  std::string y("hello");
  double z(3.14);
  EXPECT_EQ("10-hello-3.14", abslx::StrJoin(std::make_tuple(x, y, z), "-"));

  // Faster! Faster!!
  EXPECT_EQ("10-hello-3.14",
            abslx::StrJoin(std::make_tuple(x, std::cref(y), z), "-"));
  ...
  EXPECT_EQ("0x0000000a-hell-3.",
            abslx::StrJoin(std::make_tuple(x, y, z), "-", TestFormatter()));
  EXPECT_EQ(
      "0x0000000a-hell-3.",
      abslx::StrJoin(std::make_tuple(x, std::cref(y), z), "-", TestFormatter()));
  EXPECT_EQ("0x0000000a-hell-3.",
            abslx::StrJoin(std::make_tuple(&x, &y, &z), "-",
                          abslx::DereferenceFormatter(TestFormatter())));
  EXPECT_EQ("0x0000000a-hell-3.",
            abslx::StrJoin(std::make_tuple(abslx::make_unique<int>(x),
                                          abslx::make_unique<std::string>(y),
                                          abslx::make_unique<double>(z)),
                          "-", abslx::DereferenceFormatter(TestFormatter())));
  EXPECT_EQ("0x0000000a-hell-3.",
            abslx::StrJoin(std::make_tuple(abslx::make_unique<int>(x), &y, &z),
                          "-", abslx::DereferenceFormatter(TestFormatter())));
}
