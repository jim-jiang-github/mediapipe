...
// -----------------------------------------------------------------------------
//
// This header file defines an `abslx::variant` type for holding a type-safe
// value of some prescribed set of types (noted as alternative types), and
// associated functions for managing variants.
//
// The `abslx::variant` type is a form of type-safe union. An `abslx::variant`
// should always hold a value of one of its alternative types (except in the
// "valueless by exception state" -- see below). A default-constructed
// `abslx::variant` will hold the value of its first alternative type, provided
// it is default-constructible.
//
// In exceptional cases due to error, an `abslx::variant` can hold no
// value (known as a "valueless by exception" state), though this is not the
// norm.
//
// As with `abslx::optional`, an `abslx::variant` -- when it holds a value --
...
//
// `abslx::variant` is a C++11 compatible version of the C++17 `std::variant`
...
// -----------------------------------------------------------------------------
//
// An `abslx::variant` type is a form of type-safe union. An `abslx::variant` --
...
//   abslx::variant<int, std::string> v = std::string("abc");
//
//   // A default-constructed variant will hold a value-initialized value of
//   // the first alternative type.
//   auto a = abslx::variant<int, std::string>();   // Holds an int of value '0'.
//
//   // variants are assignable.
//
//   // copy assignment
//   auto v1 = abslx::variant<int, std::string>("abc");
//   auto v2 = abslx::variant<int, std::string>(10);
//   v2 = v1;  // copy assign
//
//   // move assignment
//   auto v1 = abslx::variant<int, std::string>("abc");
//   v1 = abslx::variant<int, std::string>(10);
//
//   // assignment through type conversion
//   a = 128;         // variant contains int
//   a = "128";       // variant contains std::string
//
// An `abslx::variant` holding a value of one of its alternative types `T` holds
// an allocation of `T` directly within the variant itself. An `abslx::variant`
...
// swap()
//
// Swaps two `abslx::variant` values. This function is equivalent to `v.swap(w)`
// where `v` and `w` are `abslx::variant` types.
//
...
template <
    typename... Ts,
    abslx::enable_if_t<
        abslx::conjunction<std::is_move_constructible<Ts>...,
...
// variant_size
//
// Returns the number of alternative types available for a given `abslx::variant`
// type as a compile-time constant expression. As this is a class template, it
// is not generally useful for accessing the number of alternative types of
// any given `abslx::variant` instance.
//
// Example:
//
//   auto a = abslx::variant<int, std::string>;
//   constexpr int num_types =
//       abslx::variant_size<abslx::variant<int, std::string>>();
//
//   // You can also use the member constant `value`.
//   constexpr int num_types =
//       abslx::variant_size<abslx::variant<int, std::string>>::value;
//
//   // `abslx::variant_size` is more valuable for use in generic code:
//   template <typename Variant>
//   constexpr bool IsVariantMultivalue() {
//       return abslx::variant_size<Variant>() > 1;
//   }
//
...
// variant_alternative
//
// Returns the alternative type for a given `abslx::variant` at the passed
...
// value of any given `abslx::variant` variable.
//
// Example:
//
//   // The type of the 0th alternative is "int".
//   using alternative_type_0
//     = abslx::variant_alternative<0, abslx::variant<int, std::string>>::type;
//
//   static_assert(std::is_same<alternative_type_0, int>::value, "");
//
//   // `abslx::variant_alternative` is more valuable for use in generic code:
//   template <typename Variant>
...
//
//   using alternative_type_0
//     = abslx::variant_alternative_t<0, abslx::variant<int, std::string>>;
//   static_assert(std::is_same<alternative_type_0, int>::value, "");
template <std::size_t I, class T>
...
// Example:
//
//   abslx::variant<int, std::string> foo = 42;
//   if (abslx::holds_alternative<int>(foo)) {
...
//   int i = abslx::get<int>(a);
//
//   auto b = abslx::variant<int, int>;
//
//   // Getting the value by a type that is not unique is ill-formed.
//   int j = abslx::get<int>(b);     // Compile Error!
//
...
// Note: `abslx::move()` is required to allow use of constexpr in C++11.
template <class T, class... Types>
constexpr T&& get(variant<Types...>&& v) {
  return variant_internal::VariantCoreAccess::CheckedAccess<
      variant_internal::IndexOf<T, Types...>::value>(abslx::move(v));
}
...
constexpr variant_alternative_t<I, variant<Types...>>&& get(
    variant<Types...>&& v) {
  return variant_internal::VariantCoreAccess::CheckedAccess<I>(abslx::move(v));
}
...
// index.
template <std::size_t I, class... Types>
constexpr abslx::add_pointer_t<variant_alternative_t<I, variant<Types...>>>
...
// variant by index.
template <std::size_t I, class... Types>
constexpr abslx::add_pointer_t<const variant_alternative_t<I, variant<Types...>>>
...
// type.
template <class T, class... Types>
constexpr abslx::add_pointer_t<T> get_if(variant<Types...>* v) noexcept {
  return abslx::get_if<variant_internal::IndexOf<T, Types...>::value>(v);
}

// Overload for getting a pointer to the const value stored in the given variant
// by type.
template <class T, class... Types>
constexpr abslx::add_pointer_t<const T> get_if(
...
// visit()
//
// Calls a provided functor on a given set of variants. `abslx::visit()` is
...
//   };
//
//   // Declare our variant, and call `abslx::visit()` on it.
//   // Note that `GetVariant()` returns void in either case.
//   abslx::variant<int, std::string> foo = std::string("foo");
//   GetVariant visitor;
//   abslx::visit(visitor, foo);  // Prints `The variant's value is: foo'
template <typename Visitor, typename... Variants>
variant_internal::VisitResult<Visitor, Variants...> visit(Visitor&& vis,
                                                          Variants&&... vars) {
  return variant_internal::
      VisitIndices<variant_size<abslx::decay_t<Variants> >::value...>::Run(
          variant_internal::PerformVisitation<Visitor, Variants...>{
              std::forward_as_tuple(abslx::forward<Variants>(vars)...),
              abslx::forward<Visitor>(vis)},
          vars.index()...);
}
...
// `abslx::monostate` Relational Operators
...
//------------------------------------------------------------------------------
// `abslx::variant` Template Definition
//------------------------------------------------------------------------------
template <typename T0, typename... Tn>
class variant<T0, Tn...> : private variant_internal::VariantBase<T0, Tn...> {
  static_assert(abslx::conjunction<std::is_object<T0>,
                                  std::is_object<Tn>...>::value,
                "Attempted to instantiate a variant containing a non-object "
                "type.");
  // Intentionally not qualifying `negation` with `abslx::` to work around a bug
  // in MSVC 2015 with inline namespace and variadic template.
  static_assert(abslx::conjunction<negation<std::is_array<T0> >,
                                  negation<std::is_array<Tn> >...>::value,
                "Attempted to instantiate a variant containing an array type.");
  static_assert(abslx::conjunction<std::is_nothrow_destructible<T0>,
 ...
 public:
  ...
  // 2018. It will be implemented and integrated into `abslx::variant`.
  template <
      class T,
      std::size_t I = std::enable_if<
          variant_internal::IsNeitherSelfNorInPlace<variant,
                                                    abslx::decay_t<T>>::value,
          variant_internal::IndexOfConstructedType<variant, T>>::type::value,
      class Tj = abslx::variant_alternative_t<I, variant>,
      abslx::enable_if_t<std::is_constructible<Tj, T>::value>* =
          nullptr>
  constexpr variant(T&& t) noexcept(std::is_nothrow_constructible<Tj, T>::value)
      : Base(variant_internal::EmplaceTag<I>(), abslx::forward<T>(t)) {}
  ...
  constexpr explicit variant(in_place_index_t<I>, Args&&... args)
      : Base(variant_internal::EmplaceTag<I>(), abslx::forward<Args>(args)...) {}
  ...
  // `abslx::valueless_by_exception()` is false.
  ~variant() = default;
  ...
  template <
      class T,
      std::size_t I = std::enable_if<
          !std::is_same<abslx::decay_t<T>, variant>::value,
          variant_internal::IndexOfConstructedType<variant, T>>::type::value,
      class Tj = abslx::variant_alternative_t<I, variant>,
  ...
  // `abslx::valueless_by_exception()` is false). Requires that T is unambiguous
  // in the variant.
  //
  // Example:
  //
  //   abslx::variant<std::vector<int>, int, std::string> v;
  //   v.emplace<int>(99);
  //   v.emplace<std::string>("abc");
  template <
      class T, class... Args,
      typename std::enable_if<std::is_constructible<
          abslx::variant_alternative_t<
              variant_internal::UnambiguousIndexOf<variant, T>::value, variant>,
          Args...>::value>::type* = nullptr>
  T& emplace(Args&&... args) {
    return variant_internal::VariantCoreAccess::Replace<
        variant_internal::UnambiguousIndexOf<variant, T>::value>(
        this, abslx::forward<Args>(args)...);
  }

  // Constructs a value of the given alternative type T within the variant using
  // an initializer list. The existing value of the variant is destroyed first
  // (provided that `abslx::valueless_by_exception()` is false). Requires that T
  ...
  T& emplace(std::initializer_list<U> il, Args&&... args) {
    return variant_internal::VariantCoreAccess::Replace<
        variant_internal::UnambiguousIndexOf<variant, T>::value>(
        this, il, abslx::forward<Args>(args)...);
  }

  // Destroys the current value of the variant (provided that
  // `abslx::valueless_by_exception()` is false) and constructs a new value at
  // the given index.
  //
  // Example:
  //
  //   abslx::variant<std::vector<int>, int, int> v;
  //   v.emplace<1>(99);
  //   v.emplace<2>(98);
  //   v.emplace<int>(99);  // Won't compile. 'int' isn't a unique type.
  template <std::size_t I, class... Args,
            typename std::enable_if<
                std::is_constructible<abslx::variant_alternative_t<I, variant>,
                                      Args...>::value>::type* = nullptr>
  abslx::variant_alternative_t<I, variant>& emplace(Args&&... args) {
  ...
  template <std::size_t I, class U, class... Args,
            typename std::enable_if<std::is_constructible<
                abslx::variant_alternative_t<I, variant>,
                std::initializer_list<U>&, Args...>::value>::type* = nullptr>
  abslx::variant_alternative_t<I, variant>& emplace(std::initializer_list<U> il,
                                                   Args&&... args) {
    return variant_internal::VariantCoreAccess::Replace<I>(
        this, il, abslx::forward<Args>(args)...);
  }
  ...
  constexpr bool valueless_by_exception() const noexcept {
    return this->index_ == abslx::variant_npos;
  }
...
// hash()
template <>  // NOLINT
struct hash<abslx::monostate> {
  std::size_t operator()(abslx::monostate) const { return 0; }
};

template <class... T>  // NOLINT
struct hash<abslx::variant<T...>>
    : abslx::variant_internal::VariantHashBase<abslx::variant<T...>, void,
                                              abslx::remove_const_t<T>...> {};
...
// ConvertVariantTo()
//
// Helper functions to convert an `abslx::variant` to a variant of another set of
// types, provided that the alternative type of the new variant type can be
// converted from any type in the source variant.
//
// Example:
//
//   abslx::variant<name1, name2, float> InternalReq(const Req&);
//
//   // name1 and name2 are convertible to name
//   abslx::variant<name, float> ExternalReq(const Req& req) {
//     return abslx::ConvertVariantTo<abslx::variant<name, float>>(
//              InternalReq(req));
//   }
template <typename To, typename Variant>
To ConvertVariantTo(Variant&& variant) {
  return abslx::visit(variant_internal::ConversionVisitor<To>{},
