...
Status AllocateScopedShapedBuffer(
  ...
  *buffer = abslx::make_unique<xla::ScopedShapedBuffer>(
...
xla::StatusOr<bool> XRTTupleAllocation::SwapOut(xla::Backend* backend,
  ...
  if (literal_ == nullptr && (!IsPinned() || swap_pinned)) {
    ...
    ReleaseBuffers();
    literal_ = abslx::make_unique<xla::Literal>(std::move(literal));
    return true;
  }
  return false;
}
...
/*static*/ Status XRTTupleAllocation::MakeTuple(
  ...
  auto new_tuple_buffers = abslx::make_unique<xla::ScopedShapedBuffer>(
