  ...
  if (devices_.empty()) {
    ...
    for (const auto& kv : lfqn_map_) {
      ...
      if (parsed) {
        ...
        const auto type = abslx::AsciiStrToLower(parsed_name.type);
        if (type == "gpu") {
...
string VirtualPlacer::to_lfqn_or_empty(const string& device_name) const {
  DeviceNameUtils::ParsedName parsed_name;
  const auto lowercase_name = abslx::AsciiStrToLower(device_name);
  ...
  parsed_name.type = abslx::AsciiStrToLower(parsed_name.type);
