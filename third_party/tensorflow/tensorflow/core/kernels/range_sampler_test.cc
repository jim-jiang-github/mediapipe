...
class RangeSamplerTest : public ::testing::Test {
 protected:
  ...
  void CheckHistogram(int num_samples, float tolerance) {
    ...
    sampler_->SampleBatch(&rnd, false, abslx::MakeSpan(a));
...
TEST_F(RangeSamplerTest, All) {
  ...
  sampler_->SampleBatchGetExpectedCount(nullptr,  // no random numbers needed
                                        false, abslx::MakeSpan(batch),
                                        abslx::MakeSpan(batch_expected), extras,
                                        abslx::MakeSpan(extras_expected));
...
TEST_F(RangeSamplerTest, Unique) {
  ...
  sampler_->SampleBatchGetExpectedCount(&rnd, true, abslx::MakeSpan(batch),
                                        MutableArraySlice<float>(), all_values,
                                        abslx::MakeSpan(expected));
  ...
  for (int trial = 0; trial < num_batches; trial++) {
    ...
    sampler_->SampleBatchGetExpectedCount(
        &rnd, true, abslx::MakeSpan(batch), MutableArraySlice<float>(),
        all_values, abslx::MakeSpan(trial_expected));
    for (int i = 0; i < range; i++) {
