...
TEST_F(RollOpTest, Error_InputMustBeVectorOrHigher) {
  ...
  EXPECT_TRUE(abslx::StrContains(s.ToString(), "input must be 1-D or higher"))
      << s;
}
...
TEST_F(RollOpTest, Error_AxisMustBeScalarOrVector) {
  ...
  Status s = RunOpKernel();
  EXPECT_TRUE(
      abslx::StrContains(s.ToString(), "axis must be a scalar or a 1-D vector"))
      << s;
}
...
TEST_F(RollOpTest, Error_ShiftMustBeScalarOrVector) {
  ...
  Status s = RunOpKernel();
  EXPECT_TRUE(
      abslx::StrContains(s.ToString(), "shift must be a scalar or a 1-D vector"))
      << s;
}
...
TEST_F(RollOpTest, Error_ShiftAndAxisMustBeSameSize) {
  ...
  Status s = RunOpKernel();
  EXPECT_TRUE(
      abslx::StrContains(s.ToString(), "shift and axis must have the same size"))
      << s;
}
...
TEST_F(RollOpTest, Error_AxisOutOfRange) {
  ...
  AddInputFromArray<int32>(TensorShape({}), {1});
  Status s = RunOpKernel();
  EXPECT_TRUE(abslx::StrContains(s.ToString(), "is out of range")) << s;
}
