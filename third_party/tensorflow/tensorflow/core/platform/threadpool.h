  ...
  class SchedulingParams {
   public:
    explicit SchedulingParams(SchedulingStrategy strategy,
                              abslx::optional<int64_t> cost_per_unit,
                              abslx::optional<int64_t> block_size)
        : strategy_(strategy),
          cost_per_unit_(cost_per_unit),
          block_size_(block_size) {}

    SchedulingStrategy strategy() const { return strategy_; }
    abslx::optional<int64_t> cost_per_unit() const { return cost_per_unit_; }
    abslx::optional<int64_t> block_size() const { return block_size_; }
   ...
   private:
    ...
    // strategy.
    abslx::optional<int64_t> cost_per_unit_;

    // The block size of each shard. Only applicable for Fixed Block Size
    // scheduling strategy.
    abslx::optional<int64_t> block_size_;
  };
