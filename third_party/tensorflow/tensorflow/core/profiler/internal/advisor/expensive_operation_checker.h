 ...
 private:
  AdviceProto::Checker Check(const AdvisorOptionsProto::CheckerOption& options,
    ...
    if (stats->steps().empty()) {
      abslx::FPrintF(stderr, "Missing RunMetadata info. Skip %s\n", name());
    }
    CheckOpView(stats);
  ...
  void CheckOpView(const TFStats* stats) {
    if (stats->steps().empty()) {
      abslx::FPrintF(stderr, "Missing run_meta for %s\n", name());
      return;
    }
    ...
    for (int i = 0; i < 3 && node->children_size() > 0; ++i) {
      ...
      outputs.push_back(abslx::StrFormat(
    ...
    reports_.add_reports(abslx::StrJoin(outputs, "\n"));
  }

  void CheckCodeView(const TFStats* stats) {
    if (!stats->has_code_traces()) {
      abslx::FPrintF(stderr, "Missing op_log (code traces) for %s\n", name());
      return;
    }
  ...
  void CodeViewHelper(const MultiGraphNodeProto* node, int depth,
    ...
    for (int j = 0; j < 3 && j < node->children_size(); ++j) {
      ...
      outputs->push_back(
          abslx::StrFormat("%s%s, cpu: %s, accelerator: %s, total: %s",
