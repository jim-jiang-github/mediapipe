...
string FormatNumber(int64_t n) {
  if (n < 1000) {
    return abslx::StrFormat("%d", n);
  } else if (n < 1000000) {
    return abslx::StrFormat("%.2fk", n / 1000.0);
  } else if (n < 1000000000) {
    return abslx::StrFormat("%.2fm", n / 1000000.0);
  } else {
    return abslx::StrFormat("%.2fb", n / 1000000000.0);
  }
}

string FormatTime(int64_t micros) {
  if (micros < 1000) {
    return abslx::StrFormat("%dus", micros);
  } else if (micros < 1000000) {
    return abslx::StrFormat("%.2fms", micros / 1000.0);
  } else {
    return abslx::StrFormat("%.2fsec", micros / 1000000.0);
  }
}

string FormatMemory(int64_t bytes) {
  if (bytes < 1000) {
    return abslx::StrFormat("%dB", bytes);
  } else if (bytes < 1000000) {
    return abslx::StrFormat("%.2fKB", bytes / 1000.0);
  } else {
    return abslx::StrFormat("%.2fMB", bytes / 1000000.0);
  }
}

string FormatShapes(const std::vector<int64_t>& shape) {
  return abslx::StrJoin(shape, "x");
}
...
tensorflow::Status ReturnError(const std::vector<string>& pieces, int idx) {
  ...
  return tensorflow::Status(
      ...
      abslx::StrCat("Invalid option '", pieces[idx], "' value: '", val, "'"));
}

bool CaseEqual(StringPiece s1, StringPiece s2) {
  if (s1.size() != s2.size()) return false;
  return abslx::AsciiStrToLower(s1) == abslx::AsciiStrToLower(s2);
}
...
tensorflow::Status ParseCmdLine(const string& line, string* cmd,
  ...
  std::vector<string> pieces = abslx::StrSplit(line, ' ', abslx::SkipEmpty());
  ...
  *cmd = pieces[0];
  ...
  for (int i = 1; i < pieces.size(); ++i) {
    if (pieces[i] == string(tensorflow::tfprof::kOptions[0])) {
      if (pieces.size() <= i + 1 ||
          !abslx::SimpleAtoi(pieces[i + 1], &opts->max_depth)) {
        return ReturnError(pieces, i);
      }
      ++i;
    } else if (pieces[i] == tensorflow::tfprof::kOptions[1]) {
      if (pieces.size() <= i + 1 ||
          !abslx::SimpleAtoi(pieces[i + 1], &opts->min_bytes)) {
        return ReturnError(pieces, i);
      }
      ++i;
    } else if (pieces[i] == tensorflow::tfprof::kOptions[2]) {
      if (pieces.size() <= i + 1 ||
          !abslx::SimpleAtoi(pieces[i + 1], &opts->min_peak_bytes)) {
        return ReturnError(pieces, i);
      }
      ++i;
    } else if (pieces[i] == tensorflow::tfprof::kOptions[3]) {
      if (pieces.size() <= i + 1 ||
          !abslx::SimpleAtoi(pieces[i + 1], &opts->min_residual_bytes)) {
        return ReturnError(pieces, i);
      }
      ++i;
    } else if (pieces[i] == tensorflow::tfprof::kOptions[4]) {
      if (pieces.size() <= i + 1 ||
          !abslx::SimpleAtoi(pieces[i + 1], &opts->min_output_bytes)) {
        return ReturnError(pieces, i);
      }
      ++i;
    } else if (pieces[i] == tensorflow::tfprof::kOptions[5]) {
      if (pieces.size() <= i + 1 ||
          !abslx::SimpleAtoi(pieces[i + 1], &opts->min_micros)) {
        return ReturnError(pieces, i);
      }
      ++i;
    } else if (pieces[i] == tensorflow::tfprof::kOptions[6]) {
      if (pieces.size() <= i + 1 ||
          !abslx::SimpleAtoi(pieces[i + 1], &opts->min_accelerator_micros)) {
        return ReturnError(pieces, i);
      }
      ++i;
    } else if (pieces[i] == tensorflow::tfprof::kOptions[7]) {
      if (pieces.size() <= i + 1 ||
          !abslx::SimpleAtoi(pieces[i + 1], &opts->min_cpu_micros)) {
        return ReturnError(pieces, i);
      }
      ++i;
    } else if (pieces[i] == tensorflow::tfprof::kOptions[8]) {
      if (pieces.size() <= i + 1 ||
          !abslx::SimpleAtoi(pieces[i + 1], &opts->min_params)) {
        return ReturnError(pieces, i);
      }
      ++i;
    } else if (pieces[i] == tensorflow::tfprof::kOptions[9]) {
      if (pieces.size() <= i + 1 ||
          !abslx::SimpleAtoi(pieces[i + 1], &opts->min_float_ops)) {
        return ReturnError(pieces, i);
      }
      ++i;
    } else if (pieces[i] == tensorflow::tfprof::kOptions[10]) {
      if (pieces.size() <= i + 1 ||
          !abslx::SimpleAtoi(pieces[i + 1], &opts->min_occurrence)) {
        return ReturnError(pieces, i);
      }
      ++i;
    } else if (pieces[i] == tensorflow::tfprof::kOptions[11]) {
      if (pieces.size() <= i + 1 ||
          !abslx::SimpleAtoi(pieces[i + 1], &opts->step)) {
        return ReturnError(pieces, i);
      }
      ++i;
      ...
      opts->account_type_regexes =
          abslx::StrSplit(StripQuote(pieces[i + 1]), ',', abslx::SkipEmpty());
      ++i;
      ...
      if ((pieces.size() > i + 1 && abslx::StartsWith(pieces[i + 1], "-")) ||
          pieces.size() == i + 1) {
