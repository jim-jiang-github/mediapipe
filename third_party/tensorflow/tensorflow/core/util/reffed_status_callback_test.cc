...
TEST(TestReffedStatusCallback, CallsBackFail) {
  ...
  EXPECT_TRUE(abslx::StrContains(status.error_message(), "1"));
  EXPECT_TRUE(abslx::StrContains(status.error_message(), "2"));
}
...
TEST(TestReffedStatusCallback, MultiThreaded) {
  ...
  EXPECT_TRUE(abslx::StrContains(status.error_message(), "err"));
}
