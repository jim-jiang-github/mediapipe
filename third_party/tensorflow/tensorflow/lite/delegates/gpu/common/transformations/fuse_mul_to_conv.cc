 ...
 public:
  ...
  TransformResult ApplyToNodesSequence(const std::vector<Node*>& sequence,
    ...
    ElementwiseAttributes mul_attr =
        abslx::any_cast<ElementwiseAttributes>(mul_node.operation.attributes);
    if (!abslx::holds_alternative<Tensor<Linear, DataType::FLOAT32>>(
            mul_attr.param) &&
        !abslx::holds_alternative<float>(mul_attr.param)) {
      return {
    ...
    if (conv_node.operation.type == ToString(OperationType::CONVOLUTION_2D)) {
      Convolution2DAttributes* conv_attr =
          abslx::any_cast<Convolution2DAttributes>(
      ...
      ConvolutionTransposedAttributes* conv_attr =
          abslx::any_cast<ConvolutionTransposedAttributes>(
              &conv_node.operation.attributes);
      ...
      FullyConnectedAttributes* conv_attr =
          abslx::any_cast<FullyConnectedAttributes>(
              &conv_node.operation.attributes);
    ...
    abslx::Status status = RemoveFollowingNode(graph, &mul_node, &conv_node);
    if (!status.ok()) {
...
class MergeMulWithConvolution : public SequenceTransformation {
 public:
  ...
  TransformResult ApplyToNodesSequence(const std::vector<Node*>& sequence,
    ...
    abslx::Status status = RemovePrecedingNode(graph, &mul_node, &conv_node);
    if (!status.ok()) {
      return {TransformStatus::INVALID,
              "Unable to remove mul node after convolution: " +
                  std::string(status.message())};
...
std::unique_ptr<SequenceTransformation> NewMergeConvolutionWithMul() {
  return abslx::make_unique<MergeConvolutionWithMul>();
}

std::unique_ptr<SequenceTransformation> NewMergeMulWithConvolution() {
  return abslx::make_unique<MergeMulWithConvolution>();
}

void FuseConvolution2DWithMultiply(const ElementwiseAttributes& mul_attr,
                                   Convolution2DAttributes* attr) {
  auto mul = abslx::get_if<Tensor<Linear, DataType::FLOAT32>>(&mul_attr.param);
  auto mul_scalar = abslx::get_if<float>(&mul_attr.param);
