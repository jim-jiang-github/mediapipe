...
abslx::Status GetConfig(EGLDisplay display, const EGLint* attributes,
                       EGLConfig* config) {
  EGLint config_count;
  ...
  if (!chosen || config_count == 0) {
    return abslx::InternalError("No EGL error, but eglChooseConfig failed.");
  }
  return abslx::OkStatus();
}

abslx::Status CreateContext(EGLDisplay display, EGLContext shared_context,
  ...
  if (context == EGL_NO_CONTEXT) {
    return abslx::InternalError("No EGL error, but eglCreateContext failed.");
  }
...
abslx::Status EglContext::MakeCurrent(EGLSurface read, EGLSurface write) {
  bool is_made_current = eglMakeCurrent(display_, write, read, context_);
  RETURN_IF_ERROR(GetOpenGlErrors());
  if (!is_made_current) {
    return abslx::InternalError("No EGL error, but eglMakeCurrent failed.");
  }
  return abslx::OkStatus();
}
...
abslx::Status CreateConfiglessContext(EGLDisplay display,
                                     EGLContext shared_context,
                                     EglContext* egl_context) {
  if (!HasExtension(display, "EGL_KHR_no_config_context")) {
    return abslx::UnavailableError("EGL_KHR_no_config_context not supported");
  }
  return CreateContext(display, shared_context, EGL_NO_CONFIG_KHR, egl_context);
}

abslx::Status CreateSurfacelessContext(EGLDisplay display,
                                      EGLContext shared_context,
                                      EglContext* egl_context) {
  if (!HasExtension(display, "EGL_KHR_create_context")) {
    return abslx::UnavailableError("EGL_KHR_create_context not supported");
  }
  if (!HasExtension(display, "EGL_KHR_surfaceless_context")) {
    return abslx::UnavailableError("EGL_KHR_surfaceless_context not supported");
  }
...
abslx::Status CreatePBufferContext(EGLDisplay display, EGLContext shared_context,
                                  EglContext* egl_context) {
