...
class GlBuffer {
 public:
  ...
  template <typename T>
  abslx::Status Read(abslx::Span<T> data) const;

  // Writes data to a buffer.
  template <typename T>
  abslx::Status Write(abslx::Span<const T> data);

  // Maps GPU memory to CPU address space and calls reader that may read from
  // that memory.
  template <typename T>
  abslx::Status MappedRead(
      const std::function<abslx::Status(abslx::Span<const T>)>& reader) const;

  // Maps GPU memory to CPU address space and calls writer that may write into
  // that memory.
  template <typename T>
  abslx::Status MappedWrite(
      const std::function<abslx::Status(abslx::Span<T>)>& writer);

  abslx::Status MakeView(size_t offset, size_t bytes_size, GlBuffer* gl_buffer);

  // Makes a copy without ownership of the buffer.
  GlBuffer MakeRef();

  // Binds a buffer to an index.
  abslx::Status BindToIndex(uint32_t index) const;
...
// details.
template <typename T>
abslx::Status CreateReadWriteShaderStorageBuffer(uint32_t num_elements,
                                                GlBuffer* gl_buffer);

// Creates new shader storage buffer that will be filled with data once which
// will be used many times.
template <typename T>
abslx::Status CreateReadOnlyShaderStorageBuffer(abslx::Span<const T> data,
                                               GlBuffer* gl_buffer);

// Adapts raw Buffer::Read method to read data into a vector.
template <typename T>
abslx::Status AppendFromBuffer(const GlBuffer& buffer, std::vector<T>* data) {
  if (buffer.bytes_size() % sizeof(T) != 0) {
    return abslx::InvalidArgumentError("Buffer is not aligned");
  }
  size_t num_elements = buffer.bytes_size() / sizeof(T);
  data->resize(data->size() + num_elements);
  return buffer.Read<T>(
      abslx::MakeSpan(data->data() + data->size() - num_elements, num_elements));
}
...
abslx::Status CreatePersistentBuffer(size_t size, GlPersistentBuffer* gl_buffer);
...
template <typename T>
abslx::Status GlBuffer::Read(abslx::Span<T> data) const {
  ...
  return MappedRead<T>([this, data](abslx::Span<const T> src) {
...
template <typename T>
abslx::Status GlBuffer::Write(abslx::Span<const T> data) {
...
template <typename T>
abslx::Status GlBuffer::MappedRead(
    const std::function<abslx::Status(abslx::Span<const T> d)>& reader) const {
  ...
  return reader(abslx::MakeSpan(reinterpret_cast<const T*>(mapper.data()),
                               bytes_size_ / sizeof(T)));
}

template <typename T>
abslx::Status GlBuffer::MappedWrite(
    const std::function<abslx::Status(abslx::Span<T> d)>& writer) {
  if (bytes_size_ % sizeof(T) != 0) {
    return abslx::InvalidArgumentError("Buffer is not aligned");
  }
  gl_buffer_internal::BufferBinder binder(target_, id_);
  ...
  return writer(abslx::MakeSpan(reinterpret_cast<T*>(mapper.data()),
                               bytes_size_ / sizeof(T)));
}
