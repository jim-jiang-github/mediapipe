...
struct ErrorFormatter {
  void operator()(std::string* out, GLenum error) const {
    abslx::StrAppend(out, ErrorToString(error));
  }
};
...
abslx::Status GetOpenGlErrors() {
  ...
  if (error2 == GL_NO_ERROR) {
    return abslx::InternalError(ErrorToString(error));
  }
  std::vector<GLenum> errors = {error, error2};
  for (error = glGetError(); error != GL_NO_ERROR; error = glGetError()) {
    errors.push_back(error);
  }
  return abslx::InternalError(abslx::StrJoin(errors, ",", ErrorFormatter()));
#endif  // __EMSCRIPTEN__
}

abslx::Status GetEglError() {
  ...
  switch (error) {
    ...
    case EGL_BAD_PARAMETER:
      return abslx::InternalError("One or more argument values are invalid.");
  ...
  return abslx::UnknownError("EGL error: " + std::to_string(error));
}
