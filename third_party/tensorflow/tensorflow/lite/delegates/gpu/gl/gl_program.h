...
class GlProgram {
 public:
  ...
  static abslx::Status CreateWithShader(const GlShader& shader,
                                       GlProgram* gl_program);

  // Same as CreateWithShader but takes compiled shader in a binary form,
  // therefore compilation step is avoided.
  static abslx::Status CreateWithBinaryShader(const BinaryShader& shader,
                                             GlProgram* gl_program);
  ...
  // into this program.
  abslx::Status GetBinary(BinaryShader* binary_shader);

  abslx::Status SetParameter(const Variable& param);

  // Executes program
  abslx::Status Dispatch(const uint3& workgroups) const;
