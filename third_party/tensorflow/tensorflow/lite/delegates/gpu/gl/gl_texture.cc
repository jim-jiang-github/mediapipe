...
abslx::Status GlTexture::BindImage(uint32_t index, GLenum access) const {
  return TFLITE_GPU_CALL_GL(glBindImageTexture, index, id_, /* level = */ 0,
                            /* layered = */ GL_TRUE, layer_, access, format_);
}

abslx::Status GlTexture::BindAsReadonlyImage(uint32_t index) const {
  return BindImage(index, GL_READ_ONLY);
}

abslx::Status GlTexture::BindAsWriteonlyImage(uint32_t index) const {
  return BindImage(index, GL_WRITE_ONLY);
}

abslx::Status GlTexture::BindAsReadWriteImage(uint32_t index) const {
  return BindImage(index, GL_READ_WRITE);
}

abslx::Status GlTexture::BindAsSampler2D(uint32_t index) const {
...
abslx::Status SetTextureWrapAndFilter(GLenum target, GLenum texture_format) {
...
abslx::Status CreateReadOnlyRgba2dImageTexture(DataType data_type,
                                              const uint2& size,
                                              const void* data,
                                              size_t byte_size,
...
abslx::Status CreateReadOnlyRgba3dImageTexture(DataType data_type,
                                              const uint3& size,
                                              const void* data,
                                              size_t byte_size,
                                              GlTexture* gl_texture) {
...
abslx::Status CreateReadOnlyImageTexture(const uint2& size,
...
abslx::Status CreateReadOnlyImageTexture(const uint3& size,
                                        abslx::Span<const float> data,
                                        GlTexture* gl_texture) {
...
abslx::Status CreateReadOnlyImageTextureU8(const uint2& size,
                                          abslx::Span<const uint8_t> data,
                                          GlTexture* gl_texture) {
...
abslx::Status CreateReadOnlyImageTextureF16(const uint2& size,
...
abslx::Status CreateReadOnlyImageTextureF16(const uint3& size,
                                           abslx::Span<const uint16_t> data,
                                           GlTexture* gl_texture) {
...
abslx::Status CreateReadWriteRgbaImageTexture(DataType data_type,
                                             const uint2& size,
                                             GlTexture* gl_texture) {
  const GLenum kTarget = GL_TEXTURE_2D;
  const bool normalized = data_type == DataType::UINT8;
