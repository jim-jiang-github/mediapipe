...
class ObjectManager {
 public:
  ...
  abslx::Status RegisterBuffer(uint32_t id, GlBuffer buffer);
  ...
  abslx::Status RegisterTexture(uint32_t id, GlTexture texture);
...
// tensor.
abslx::Status CopyFromPHWC4Buffer(const GlBuffer& buffer, TensorFloat32* tensor);
