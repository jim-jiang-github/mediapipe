...
class Runtime {
 ...
 private:
  abslx::Status AllocateInternalObject(const Object& object);

  abslx::Status AllocateConstObject(const Object& object, uint32_t* id);
  ...
  abslx::Status AssignInternalObjects(std::vector<Object>* objects);
  ...
  using BindFunc = std::function<abslx::Status()>;
