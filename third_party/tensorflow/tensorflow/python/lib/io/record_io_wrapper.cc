...
class PyRecordReader {
 public:
  ...
  tensorflow::Status Reopen() {
    ...
    reader_ =
        abslx::make_unique<tensorflow::io::RecordReader>(file_.get(), options_);
    return ::tensorflow::OkStatus();
  }
...
class PyRecordRandomReader {
 public:
  static tensorflow::Status New(const std::string& filename,
    ...
    options.buffer_size = kReaderBufferSize;
    auto reader =
        abslx::make_unique<tensorflow::io::RecordReader>(file.get(), options);
...
class PyRecordWriter {
 public:
  static tensorflow::Status New(
    ...
    auto writer =
        abslx::make_unique<tensorflow::io::RecordWriter>(file.get(), options);
