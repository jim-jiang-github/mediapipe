...
constexpr abslx::Duration kMinSessionGraceTime = abslx::Seconds(60);

tensorflow::Status ValidateHostPortPair(abslx::string_view host_port) {
  tensorflow::uint32 port;
  std::vector<abslx::string_view> parts = abslx::StrSplit(host_port, ':');
  // Must be host:port, port must be a number, host must not contain a '/',
  // host also must not be empty.
  if (parts.size() != 2 || !abslx::SimpleAtoi(parts[1], &port) ||
      abslx::StrContains(parts[0], "/") || parts[0].empty()) {
...
tensorflow::Status ValidateOptions(
  ...
  for (abslx::string_view host_port : options.service_addresses()) {
...
void AddServiceAddresses(abslx::string_view service_addresses,
                         RemoteProfilerSessionManagerOptions* options) {
  for (abslx::string_view server : abslx::StrSplit(service_addresses, ',')) {
...
void UpdateMaxSessionDuration(RemoteProfilerSessionManagerOptions& options) {
  ...
  auto profile_duration = std::max(
      kMinSessionGraceTime, abslx::Milliseconds(local_profiler_duration) * 2);
  abslx::Duration delay_duration;
  // When requested start timestamp is 0, profiling starts immediately.
  if (requested_start_ts > 0) {
    delay_duration =
        abslx::Nanoseconds(requested_start_ts - session_creation_ts);
  }

  auto max_session_duration = profile_duration + delay_duration;
  options.set_max_session_duration_ms(
      abslx::ToInt64Milliseconds(max_session_duration));
  VLOG(1) << "max_session_duration set to " << max_session_duration;
}

// Takes profiler options in abslx::flat_hash_map and returns a
// RemoteProfilerSessionManagerOptions.
RemoteProfilerSessionManagerOptions GetOptionsLocked(
    abslx::string_view logdir,
    const abslx::flat_hash_map<std::string, abslx::variant<int>>& opts) {
  RemoteProfilerSessionManagerOptions options;
  *options.mutable_profiler_options() =
  ...
  auto now = abslx::Now();
  options.set_session_creation_timestamp_ns(abslx::ToUnixNanos(now));
  ...
  for (const auto& kw : opts) {
    abslx::string_view key = kw.first;
    if (key == "host_tracer_level") {
      int value = abslx::get<int>(kw.second);
...
RemoteProfilerSessionManagerOptions GetOptionsLocked(
    abslx::string_view service_addresses, abslx::string_view logdir,
    abslx::string_view worker_list, bool include_dataset_ops,
    int32_t duration_ms,
    const abslx::flat_hash_map<std::string, abslx::variant<int>>& opts,
