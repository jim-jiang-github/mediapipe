...
#define OSTREAM_ROCM_ERROR(__name) \
  ...
  switch (result) {
    ...
    default:
      return abslx::StrCat("hipError_t(", static_cast<int>(result), ")");
  }
}
...
static port::Status InternalInit() {
  ...
  return port::Status{port::error::ABORTED,
                      abslx::StrCat("failed call to hipInit: ", ToString(res))};
}
...
/* static */ port::Status GpuDriver::GetDevice(int device_ordinal,
  ...
  return port::Status{
      port::error::INTERNAL,
      abslx::StrCat("failed call to hipDeviceGet: ", ToString(res))};
}
...
/* static */ port::Status GpuDriver::LaunchKernel(
    GpuContext* context, abslx::string_view kernel_name, hipFunction_t function,
  ...
  GetDriverExecutor()->Schedule([context, hsaco_contents, module, &ret,
    ...
    if (res != hipSuccess) {
      ret = port::InternalError(
          abslx::StrCat("Failed to load HSACO: ", ToString(res)));
      notification.Notify();
    }
...
/* static */ port::Status GpuDriver::SynchronousMemsetUint32(
    ...
    size_t uint32_count) {
  ScopedActivateContext activation{context};
  void* pointer = abslx::bit_cast<void*>(location);
  RETURN_IF_ROCM_ERROR(
...
/* static */ void GpuDriver::DeviceDeallocate(GpuContext* context,
                                              void* location) {
  ScopedActivateContext activation{context};
  hipDeviceptr_t pointer = abslx::bit_cast<hipDeviceptr_t>(location);
  ...
  switch (res) {
    ...
    case hipErrorNotInitialized:
      return port::Status{
          ...
          abslx::StrFormat("error destroying ROCM event in device %d: %s",
...
/* static */ port::Status GpuDriver::RecordEvent(GpuContext* context,
  ...
  switch (res) {
    ...
    case hipErrorNotInitialized:
      return port::Status{
          port::error::FAILED_PRECONDITION,
          abslx::StrFormat("error recording ROCM event on stream %p: %s", stream,
                          ToString(res).c_str())};
    default:
      return port::Status{
...
/* static */ port::StatusOr<hipError_t> GpuDriver::QueryEvent(
  ...
  if (res != hipSuccess && res != hipErrorNotReady) {
    return port::Status{
        port::error::INTERNAL,
        abslx::StrFormat("failed to query event: %s", ToString(res).c_str())};
  }
...
/* static */ bool GpuDriver::AsynchronousMemcpyD2H(GpuContext* context,
  ...
  if (res != hipSuccess) {
    LOG(ERROR) << abslx::StrFormat(
        ...
        "Gpu src: %p; size: %llu=0x%llx",
        ToString(res).c_str(), host_dst, abslx::bit_cast<void*>(gpu_src), size,
        size);
    return false;
  }
...
/* static */ bool GpuDriver::AsynchronousMemcpyH2D(GpuContext* context,
  ...
  if (res != hipSuccess) {
    LOG(ERROR) << abslx::StrFormat(
        ...
        "host src: %p; size: %llu=0x%llx",
        ToString(res).c_str(), abslx::bit_cast<void*>(gpu_dst), host_src, size,
        size);
    return false;
  }
...
/* static */ port::Status GpuDriver::InitEvent(GpuContext* context,
  ...
  if (res == hipSuccess) {
    ...
    return port::Status{
        port::error::FAILED_PRECONDITION,
        abslx::StrCat("could not create ROCM event: ", ToString(res))};
  }
}
...
/* static */ port::Status GpuDriver::GetComputeCapability(int* cc_major,
  ...
  return port::Status(
      port::error::INTERNAL,
      abslx::StrFormat("failed to get compute capability for device: %d "
  ...
  if (result == hipSuccess) {
    switch (value) {
      ...
      default:
        return port::Status{
            port::error::INTERNAL,
            abslx::StrCat("unknown memory space provided by ROCM API: ", value)};
    }
  }
...
/* static */ port::StatusOr<hipDevice_t> GpuDriver::GetPointerDevice(
  ...
  if (result != hipSuccess) {
    return port::Status{
        port::error::INTERNAL,
        abslx::StrCat("failed to get device for pointer: ", ToString(result))};
  }
...
/* static */ port::Status GpuDriver::GetGpuISAVersion(int* version,
  ...
  *version = 0;
  return port::Status{
      port::error::INTERNAL,
      abslx::StrFormat("failed to determine AMDGpu ISA version for device %d",
                      device)};
}
...
/* static */ port::Status GpuDriver::GetGpuGCNArchName(
  ...
  *gcnArchName = "";
  return port::Status{
      port::error::INTERNAL,
      abslx::StrFormat("failed to determine AMDGpu GCN Arch Name for device %d",
                      device)};
}
...
static port::StatusOr<T> GetSimpleAttribute(hipDevice_t device,
  ...
  if (result != hipSuccess) {
    return port::Status{
        ...
        abslx::StrCat("could not retrieve ROCM device attribute (", attribute,
                     "): ", ToString(result))};
  }
  T converted = value;
  return converted;
...
/* static */ port::Status GpuDriver::EnablePeerAccess(GpuContext* from,
  ...
  if (result != hipSuccess && result != hipErrorPeerAccessAlreadyEnabled) {
    return port::Status{
        port::error::INTERNAL,
        abslx::StrFormat("failed to enable peer access from %d to %d: %s",
...
/* static */ port::StatusOr<int> GpuDriver::GetMaxOccupiedBlocksPerCore(
  ...
  if (result != hipSuccess) {
    return port::Status{
        port::error::INTERNAL,
        abslx::StrFormat("failed to calculate occupancy of kernel %p: %s",
