...
void PrintBenchmarkUsage(const std::vector<const NodeDef*>& placeholders,
  ...
  for (const NodeDef* node : all_inputs) {
    ...
    if (shape.dims() == -1) {
      ...
      sizes_string = abslx::StrJoin(sizes, ",");
    }
  ...
  string input_layer_value = abslx::StrJoin(input_layers, ",");
  string input_layer_type_value = abslx::StrJoin(input_layer_types, ",");
  string input_layer_shape_value = abslx::StrJoin(input_layer_shapes, ":");
  string output_layer_value = abslx::StrJoin(output_layers, ",");
